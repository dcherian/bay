...
#+NAME: tab:pods
#+ATTR_LATEX: :placement [t] :environment tabular* :width \textwidth
...
The seasonal cycle of thermocline turbulence is dramatic with near-molecular values of diffusivity observed during the NESW transition period --- mean $K_T \le \SI{1e-5}{\m\squared\per\s}$  and median $K_T \le \SI{1e-6}{\m\squared\per\s}$ for depths deeper than \SI{50}{m} during March and April in Figure ref:fig:vert.
...
\begin{equation}
K_T^\text{min} \approx K_ρ^\text{min} = \frac{Γε^\text{min}}{N²} \approx \frac{0.2 × \SI{1e-9}{\W\per\kg}}{\SI{1e-3}{\per\s\squared}}  \approx\SI{2e-7}{\m\squared\per\s},
\end{equation}
assuming again that mixing efficiency $Γ = 0.2$ citep:Gregg2018.
Low thermocline diffusivities are predicted by the finestructure internal-wave scaling of cite:Henyey1986 and have been observed previously at low latitudes: $K_ρ \approx \SIrange[fixed-exponent=-6, scientific-notation=fixed,range-units=brackets]{1e-6}{3e-6}{\square\m\per\s}$ for latitudes south of 10N in cite:Gregg2003.
...
The exported water is saline with $S\approx34-35$, necessitating an inflow of salty Arabian Sea water citep:Vinayachandran2013.
...
Given the data coverage, we define the salty Arabian Sea water mass as parcels with salinity $S > 34.75$ since we have year-long observations in the $35 \le S \le 34.5$ salinity bin (Figure [[fig:8njs]]b).
