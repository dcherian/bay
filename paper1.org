...
This process yields the turbulence dissipation rate of temperature variance $χ$ after relating $χ$ to $ε$, turbulence dissipation rate of kinetic energy, by assuming $K_T = K_ρ = Γε/N^2$ i.e. that the turbulence diffusivities of temperature and density are equal, and that mixing efficiency $Γ = 0.2$ for stratified turbulence citep:Moum2009a.
...
Again, we have assumed thath igh Reynolds number geophysical turbulence mixes all scalars at the same rate so that the diffusivities of both temperature and salinity are equal, $K_T = K_S$.
...
/Eulerian/ variance-preserving rotary spectra of the WKB-scaled shear time-series at all NRL moorings were dominated by a broad peak at $-f_0$, narrow secondary peaks at $-f_0 \pm ω_{M2}$ ($ω_{M2}$ is the $M_2$ tidal frequency) and significant energy at frequencies $\le \SI{20}{days}$ reflecting meanders of the Summer Monsoon Current (primarily at NRL3).
.
