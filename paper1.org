#+TITLE:  Drivers and observations of the seasonal cycle of upper-ocean mixing in the Bay of Bengal
...
Now we examine the drivers of the observed seasonal cycle in turbulent mixing.
For a broad perspective, we explore variations of near-surface mixing (bins `ML', `BL' in Figure ref:fig:vert) with seasonally-varying wind stress and thermocline mixing (deeper density bins) with seasonal cycle in the low-frequency and the near-inertial shear fields.
...
The 105-m χpod at NRL5 observes mixing that is weaker by one to two orders of magnitude relative to the RAMA 12°N 15m χpod discussed previously section (compare Figure [[fig:rama]]b and Figure [[fig:nrl]]b).
...
Even then diffusivity $K_T$ at NRL5 is consistently below and rarely exceeds the canonical thermocline value of \SI{1e-5}{\m\square\per\s} (Figure [[fig:nrl]]b).
...
Both seasonal mean surface velocities from OSCAR and mooring ADCP data show the SMC to be prominent especially at NRL3 and NRL4, the two westernmost moorings along 8°N (EKE max in Figure ref:fig:spatial, top row and citealp:Wijesekera2016a).
...
These bursts tend to coincide with enhanced mixing events similar to other work that has shown a link between wind-forced near-inertial waves and thermocline mixing (for example, citealp:Alford2001).

We now discuss the seasonal cycle of shear and mixing by presenting diffusivity $K_T$ as a function of both low-frequency and near-inertial shear squared in Figure ref:fig:shearscatter.
...
With the onset of the SW monsoon, χpods on the NRL moorings observe an order of magnitude increase in mean /thermocline/ diffusivity to $K_T ≈ \SI{1e-4}{\m\squared\per\s}$ with peak values of $K_T≈\SI{1e-2}{\m\squared\per\s}$ between May and September.
...
These mixing events generally correspond to energetic near-inertial shear events (Figure [[fig:shearscatter]]d) that are likely forced by the two strong wind events at the surface in October and November (Figure [[fig:nrl]]a).
Surface geostrophic velocities in the OSCAR dataset suggest that the SMC ceases to exist as a continuous flow from the Arabian Sea at the end of September.
...
Despite the absence of an organized mean flow, high salinity and relatively weakly-stratified water is still present in the south-central Bay (Figures [[fig:nrl]]d and ref:fig:spatial).
-  + some mixing
...
The seasonal variability in near-inertial shear variance weakly mirrors the seasonal cycle of /local/ near-inertial input (Figure ref:fig:spatial).
...
Given the weak relation between local near-inertial shear variance and local near-inertial input, we leave a more detailed examination of Bay's near-inertial field to a future paper and instead focus on relating the observed mixing to local near-inertial events.
A scatter plot of monthly mean $K_T$ against monthly mean $Π$ did not reveal a meaningful relationship between the two quantities unlike in cite:Whalen2018.
This likely reflects the importance of mean shear to .
...
#+CAPTION: Drivers of thermocline mixing at the NRL moorings in the south-central Bay: the seasonal cycle of squared shear ($S²$) and diffusivity $K_T$. Each panel represents one season and shows the two-dimensional distribution of the bin-average of hourly mean $K_T$ (color) against near-inertial squared shear (y-axis) and low-frequency squared shear (x-axis).
...
- reference drew's thing about near-inertial penetration
