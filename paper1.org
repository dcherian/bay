...
Turbulence temperature diffusivity $K_T$, turbulence heat flux $J_q^t$ and turbulence salt flux $J_s^t$ are estimated from a time series of $χ$ using
\begin{equation}
        K_T = \frac{χ/2}{T_z^2}; \quad J_q^t = - ρ_0\, c_p\, K_T\, T_z; \quad J_s^t = ρ_0 \, K_T \, S_z
\end{equation}
where $T_z, S_z$ are background temperature and salinity gradients obtained from nearby CTDs on the moorings (subscript $z$ indicates \(z\)-derivative).
...
Inferring $K_T$ and $J_q$ when temperature stratification $T_z$ is small can be problematic since these quantities are inversely proportional to $T_z^2$ and $T_z$ respectively.
Such behaviour is especially problematic in the Bay where salinity often dominates near-surface stratification, unlike in the equatorial cold tongues where χpods are most commonly deployed.
...
(setq ob-jupyter-python-suppress-execution-count t)
(setq ob-jupyter-python-show-mime-types nil)
#+END_SRC

#+BEGIN_SRC jupyter-python :session py :exports results :results replace  drawer :eval never-export :display text/plain
import tabulate
...
for index, row in moorcsv.iterrows():
    table['KT_moor_mean'].iloc[index] = (f"{row.KT_mean*factor:.1f}"
                                         f"_{{{row.ci_mean[0]*factor:.2f}}}"
                                         f"^{{{row.ci_mean[1]*factor:.2f}}}")
    table['KT_moor_median'].iloc[index] = (f"{row.KT_median*factor:.1f}"
...
for index, row in sortcsv.iterrows():
    ...
    table['KT_sort_mean'].iloc[index] = (f"{row.KT_mean*factor:.1f}"
                                         f"_{row.ci_mean[0]*factor:.2f}"
                                         f"^{row.ci_mean[1]*factor:.2f}")
    table['KT_sort_median'].iloc[index] = (f"{row.KT_median*factor:.1f}"
                                           f"_{row.ci_median[0]*factor:.2f}"
                                           f"^{row.ci_median[1]*factor:.2f}")
...
neat_table = (pd.pivot_table(table,
                             ...
                             columns='season',
                             aggfunc=lambda x: x)
              [['KT_sort_median', 'KT_moor_median', 'KT_sort_mean', 'KT_moor_mean']]
              .reindex(['ML', 'BL'] + list(sortcsv.bin.unique()[:-2])))

(neat_table.rename(columns={'KT_sort_median': 'median (sorted)',
                            'KT_sort_mean': 'mean (sorted)',
                            'KT_moor_median': 'median (mooring)',
                            'KT_moor_mean': 'mean (mooring)'})
 .to_latex(escape=False,
           column_format='@{\extracolsep\\fill}r|ssss|ssss|ssss|ssss',
...
#+RESULTS:
: \begin{tabular}{@{\extracolsep\fill}r|ssss|ssss|ssss|ssss}\n\toprule\n{} & \multicolumn{4}{c}{median (sorted)} & \multicolumn{4}{c}{median (mooring)} & \multicolumn{4}{c}{mean (sorted)} & \multicolumn{4}{c}{mean (mooring)} \\\nseason &              NE &           NESW &                SW &           SWNE &                NE &             NESW &                SW &             SWNE &                 NE &              NESW &                SW &             SWNE &                       NE &                      NESW &                       SW &                    SWNE \\\nbin              &                 &                &                   &                &                   &                  &                   &                  &                    &                   &                   &                  &                          &                           &                          &                         \\\n\midrule\nML               &   7.7_7.32^8.07 &  1.1_1.00^1.26 &  13.1_12.60^13.52 &  2.5_2.33^2.67 &  54.7_49.10^60.19 &    3.3_2.56^4.16 &  37.1