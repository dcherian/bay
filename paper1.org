...
Here we summarize year-long observations of turbulence at multiple sites in the Bay (Figure ref:fig:map) and find that the seasonal cycle of winds, currents and near-inertial energy is imprinted on both near-surface and thermocline turbulence (Sections ref:sec:profile).
...
\end{equation}
where $κ_T$ is the molecular diffusivity of water and $T_z, S_z$ are background temperature and salinity gradients (usually obtained by differencing nearby CTDs on the moorings --- /mooring gradients/ $T_z\moor, S_z\moor$; subscript $z$ indicates \(z\)-derivative).
Again, we have assumed that high Reynolds number geophysical turbulence mixes all scalars at the same rate so that the diffusivities of both temperature and salinity are equal, $K_T = K_S$.
...
Such behaviour is especially problematic in the Bay where salinity often dominates near-surface stratification and temperature inversions are common (for e.g. citealp:Shroyer2016), unlike in the equatorial cold tongues where χpods are most commonly deployed (e.g. citealp:Perlin2012 where $T_z \ge \SI{1e-2}{\celsius \per\m}$).
...
When the recorded temperature variance is within an arbitrary factor of 1.5 of the inferred noise variance, we set $ε$ to NaN and $χ$ to 0 resulting in total diffusivity $K_T$  being floored to molecular diffusivity $κ_T$ and the resulting heat flux $J_q^t$ being that due to molecular diffusion.
...
5) The PDFs are presented at the mean depth of the density bin as a vertical profile (Figure ref:fig:vert). Each PDF is also labelled with either `ML', `BL', or the mean density in each bin as appropriate. The vertical bars indicate the standard deviation of depths of observation; these frequently overlap each other. Circles and triangles indicate medians and means respectively. The horizontal line at the base of each PDF spans the range between the minimum observed value and the $99^{\text{th}}$ percentile. The PDFs are colored so as to indicate amount of data in each bin (see caption).
...
Here we examine the drivers of the observed seasonal cycle in turbulent mixing depicted in Figure ref:fig:vert.
...
The seasonal cycle of thermocline turbulence is dramatic with near-molecular values of $K_T$ observed during the NESW transition period --- mean $K_T \le \SI{1e-5}{\m\squared\per\s} (50κ_T)$ and median $K_T \le \SI{1e-6}{\m\squared\per\s} (5κ_T)$ for depths greater than \SI{50}{m} during March and April, assuming a typical value of molecular diffusivity $κ_T = \SI{2e-7}{\m\squared\per\s}$ (Figure ref:fig:vert).
...
/Eulerian/ variance-preserving rotary spectra of the WKB-scaled shear time-series at all NRL moorings were dominated by a broad peak at $-f_0$ with secondary peaks at $-f_0 \pm ω_{M2}$ ($ω_{M2}$ is the $M_2$ tidal frequency).
These secondary peaks are a sign of vertical pumping of near-inertial shear by the internal $M_2$ tide which Doppler-shifts some spectral energy from $-f_0$ to $-f_0 \pm ω_{M2}$ citep:Alford2001a.

We present an example of pumping of shear layers and resulting modulation of $K_T$ at $M_2$ period in Figure [[ref:fig:nrl5-niw]] for July 23 -- August 7, when maximum mixing is observed at NRL5 (highlighted in white in Figure [[fig:nrl]]b,c).
...
The Eulerian shear spectra also indicated significant energy at frequencies $\le \SI{20}{days}$ especially at NRL3, reflecting meanders of the Summer Monsoon Current.
...
Instead we account for tidal pumping by further filtering the shear time series in a narrow band about $M_2$ and adding that to $S\niw$ i.e. all shear variance near $M_2$ is attributed to near-inertial waves.
...
Coarse sampling severely limits our ability to confidently infer a seasonal cycle in Richardson number $\Ri = N^2/u_z^2$ estimated using 20m-scale shear (not WKB scaled) and $N^2$ sampled hourly[fn::Similarly reduced shear $S^2 - 4N^2$ is rarely positive and not useful.].
...
\begin{equation}
K_ρ^\text{min} = \frac{Γε^\text{min}}{N^2} \approx \frac{0.2 \times 1 \times \SI{1e-9}{\W\per\kg}}{\SI{1e-3}{\per\s\squared}}  \approx\SI{2e-7}{\m\squared\per\s} \approxeq κ_T,
\end{equation}
...
There is a strong seasonal cycle in thermocline mixing that appears to correspond to a seasonal cycle in thermocline shear.
...
That said Figure ref:fig:shearscatter indicates that thermocline mixing is correlated with enhanced near-inertial shear throughout the year, and with low-frequency shear during the SW monsoon.
...
This value is an order of magnitude larger than the mean $K_T \approxeq (1 \mhyph 3) κ_T$ we observe between \SIrange{80}{100}{m} at 8°N in the Bay during March and April (Table ref:tab:means).
...
Another puzzle is the extended period of low mixing during March and April — is the internal field weaker than that expected from the Garrett-Munk spectrum and if so, why?
