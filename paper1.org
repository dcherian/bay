...
def clean_axis(ax):
    bay.mark_moors(ax=ax, markersize=4.5, fontsize=6, color='k',
                   colortext='w', labels=False)
    bay.mark_moors(ax=ax, markersize=3, fontsize=6, color='w',
                   colortext='w', labels=False)
    ax.set_xlabel('')
    ax.set_ylabel('')
...
hcb2 = plt.colorbar(hniw, ax=ax[0, 2:], orientation='horizontal',
                    label='Near-inertial wind input $Π$ [mW/m²]',
...
import subprocess
subprocess.run('bash images/add-legend-vert-profile.bash')
#+END_SRC
...
grouped = df.groupby([pd.cut(df.S, [34, 34.5, 35, 36]),
                      df.time.dt.month])
...
f2, ax = plt.subplots(1, 1, constrained_layout=True, squeeze=True)
f2.set_size_inches(7, 2.6)
sns.heatmap((mean['Js'] * 1000)
            .where(count > N)
            .reset_index().pivot('S', 'time', 'Js'),
            cmap=cmocean.cm.matter, robust=True, vmax=1,
            square=True, annot=True, cbar=False, linewidths=1, fmt='.2f')
ax.set_title('$J_s^t$ [$10^{-3}$ g/m²/s]')
...
ax.set_xticklabels(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul',
                    'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
ax.set_xlabel('2014')
f2.savefig('images/paper1/8N-binned-avg-Js.pdf', bbox_to_inches='tight')
#+END_SRC

[[file:images/paper1/8N-binned-avg-Js.pdf]]

** Figure 5: RAMA 12N time series
...
hjq = (nrl5.Jq.sel(time=trange).isel(depth=1)
       .resample(time='2H', loffset='-1H').mean('time')
       .plot.line(x='time', ax=ax[0], ylim=[-50, 0], add_legend=True, color='k'))
# hjq[0].set_clip_on(False)
# hjq[0].set_in_layout(False)

(nrl5.ε.sel(time=trange).isel(depth=1)
 .plot.line(x='time', ax=ax[1], yscale='log', ylim=[1e-11, 1e-6],
            add_legend=False, color='k', lw=0.5))
...
dcpy.plots.annotate_end(hz[0], 'χpod')
