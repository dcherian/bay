...
\newpage
\begin{abstract}
We describe the seasonal cycle of turbulent mixing in the top \SI{100}{m} of the Bay of Bengal as observed by moored turbulence sensors (χpods) during 2014 and 2015.
...
In the thermocline at two moorings, monthly mean turbulent kinetic energy dissipation $ε$ is correlated with monthly mean squared near-inertial shear but this agreement breaks down at two other moorings.
We also find that monthly averaged turbulent salt flux out of the salty Arabian Sea water mass being advected northward by the Summer Monsoon Current at 8°N between August and January can be significant relative to local $E-P$ and is of the right magnitude to make up the residual salt flux required by multiple model-based salt budgets for the upper Bay of Bengal.
\end{abstract}
#+END_EXPORT
...
In agreement with this idea, model studies have implicated vertical mixing as the primary mechanism of diluting the immense amount of freshwater received during the southwest monsoon citep:Akhil2014,Benshila2014,Wilson2016.
...
Vertical profiles of both mean and median values of $K_T$ are always surface intensified.
The northeast and southwest monsoons are periods of relatively high mixing in the top 100m at all observed locations.
Enhanced mixing is also observed during October and November, likely a consequence of storm activity that is evident by the near-zero mean and large standard deviation of basin-wide wind stress (Figure ref:fig:vert, top row).
...
The seasonal cycle of near-surface turbulence is readily explained by the seasonal cycle of wind stress and net surface heat flux as reported earlier by cite:Warner2016.

- we really don't characterize the mixed layer during the transition

Mixing in the mixed and barrier layers is proportional to the wind stress $τ$ (Figure ref:fig:mlbl)
...
The seasonal cycle of turbulence in the thermocline is dramatic with near-molecular values of diffusivity observed during the NESW transition period --- mean $K_T ≤ \SI{1e-5}{\m\squared\per\s}$  and median $K_T ≤ \SI{1e-6}{\m\squared\per\s}$ for depths deeper than \SI{50}{m} during March and April in Figure ref:fig:vert.
The most striking feature of Figure ref:fig:vert is the near-complete lack of mixing in the south-central Bay's thermocline during the month of April --- a period of low winds, high net surface heat flux and low near-inertial energy flux.
More generally, /median/  $K_T ≤ \SI{1e-6}{\m\squared\per\s}$ in most thermocline density bins.
...
#+LATEX: \note{briefly discuss shear field ≈ SMC + near-inertial. then describe monsoons + post-monsoon profiles}
Outside of March and April, mean diffusivity in the thermocline is approximately \SI{1e-4}{\m\squared\per\s}.
Assuming that shear instability is the root cause of thermocline turbulence, two factors are possibly responsible for enhancing shear in the thermocline between May and November.
The first factor is the strongly sheared Summer Monsoon Current (SMC) meandering through the mooring line at 8°N, elevating shear and decreasing $N²$ in the top \SIrange{100}{150}{m} between July and December.
Both seasonal mean surface velocities from OSCAR and mooring ADCP data show the SMC to be prominent especially at NRL3 and NRL4, the two westernmost moorings along 8°N (Figure ref:fig:spatial and citealp:Wijesekera2016a).
The second factor is the likely presence of a seasonal cycle of near-inertial shear variance forced by the seasonal cycle of near-inertial input (Figure ref:fig:spatial), as has been observed elsewhere citep:Alford2001,Whalen2018.

We present the seasonal variation of both diffusivity $K_T$ as well as low-frequency and near-inertial shear squared in Figure ref:fig:shearscatter.
We are unfortunately restricted to estimating shear only at the deeper χpod on each mooring because the shallower χpod was deployed within the blanking zone of the downward looking ADCP.
In constructing Figure ref:fig:shearscatter we assume that shear at the shallower χpod is identical to that at the deeper χpod -- these are separated by \SI{20}{m}.
Shear at the deeper χpod is estimated by central differencing the velocity over three bins that span that χpods depth level.
We account for seasonal as well as regional variability in $N²$ by WKB scaling the shear as $u_z\wkb = u_z (\tilde{N}(t)/N_0)^{3/2}$ using $\tilde{N}(t)$  calculated as the 30-day low-passed filtered $N$ and $N_0$ calculated as the annual mean $N$.
The WKB-scaled shear time series is then filtered using a fourth-order Butterworth filter to obtain low frequency shear and near-inertial shear (lowpass with half power point 9 days; bandpass between half power points 7 days and 2 days respectively).
During the SW monsoon (May - Sep) χpods on the NRL moorings observe an order of magnitude increase in mean /thermocline/ diffusivity to $K_T ≈ \SI{1e-4}{\m\squared\per\s}$ with peak values of $K_T≈\SI{1e-2}{\m\squared\per\s}$.
Energetic mixing events coincide with the presence of either enhanced mean shear (SMC), near-inertial shear or both (Figure [[fig:shearscatter]]c).
Relatively high mean $K_T$ in bins beneath the 45° line suggests that enhanced mixing is more likely when low frequency shear is present.
At NRL3 and NRL4, energetic near-inertial events tend to coincide with enhanced low-frequency shear (note that bins spanning two decades lie along the 45° line in Figure [[fig:shearscatter]]c).
...
These mixing events generally correspond to energetic near-inertial shear events (Figure [[fig:shearscatter]]d) though there is some low frequency shear that appears to be associated with westward propagating Rossby waves (seen in OSCAR surface velocity data).
The transition months of March and April are a time of relatively low near-inertial shear as well as relatively low low-frequency shear, perhaps offering an explanation for the extremely low values of diffusivity in Figure ref:fig:vert.

#+LATEX: \note{Talk about 34.75 + point to section \ref{sec:8n}}
Finally, consider the climatological depth of the $S=34.75$ surface at 8°N relative to the seasonal variation of thermocline diffusivity in Figure ref:fig:vert (bottom row, thick black line).
# Using a basin-wide salinity budget for the upper \SI{30}{m} of the Bay, cite:Wilson2016 suggest that a domain-averaged diffusivity of \SI{1e-4}{m²\per\s} between September and November as an upper bound on turbulent mixing required to close that budget.
Mean diffusivity isohaline is approximately $\SI{1e-4}{\m\squared\per\s}$ during the SW monsoon and the post-monsoon period (SWNE) i.e. between May and November.
...
Figure ref:fig:vert suggests that the seasonally enhanced mixing in the south-central Bay's thermocline between May and November drives an upward flux of salt out of salty Arabian Sea water whose upper boundary we define using the 34.75 isohaline.
We will examine this salt flux in Section [[sec:8n]].
Next we take a closer look at the seasonal cycle of both forcing and turbulence in two χpod records: one near-surface and one in the thermocline.
...
*** Figure 5: NRL scatter plot                                    :ignore:

#+BEGIN_SRC ipython :session
import facets

subset = (turb.where(turb.lon < 89).where(subset.lat > 4.5)
          .dropna('lon', how='all')
          .dropna('lat', how='all'))
...
for seas in axes:
    ...
    mappable = ax.hexbin(_select((np.abs(low.wkb_uz + 1j*low.wkb_vz)**2),
                         ...
                         _select(_process(np.abs(niw.wkb_uz + 1j*niw.wkb_vz)**2),
                                 seas).values.ravel(),
                         C=_select(deep.KT, seas).values.ravel(),
                         reduce_C_function=np.nanmean,
                         xscale='log', yscale='log',
                         cmap=mpl.cm.plasma, gridsize=25, mincnt=10,
                         extent=[-7, -3, -7, -3],
...
#+CAPTION: Drivers of thermocline mixing at the NRL moorings in the south-central Bay. Each panel represents one season and shows the two-dimensional distribution of the bin-average of hourly mean $K_T$ (color) against near-inertial squared shear (y-axis) and low-frequency squared shear (x-axis).
...
Turbulent heat flux $J_q^t$ likewise is generally small and exceeds \SI{10}{\W\per\m\square} only for a few days (Figure [[fig:nrl]]c) in the entire year.
...
This hypothesis is supported by the surface-intensified nature of observed mixing and the seasonal variation of the depth of the $S=34.75$ isohaline along 8°N (Figure ref:fig:vert and Figure ref:fig:spatial).
...
#+LATEX: \note{weak mixing + ocean/climate simulation implications.}
...
cite:Wilson2016a find that ``negative salinity biases at 50-m depth are associated with positive salinity biases near the surface.'' between February and May in a HYCOM assimilative simulation of the Bay.
