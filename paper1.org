...
def clean_axis(ax):
    bay.plots.mark_moors_clean(ax=ax)
    ax.set_xlabel('')
    ax.set_ylabel('')
...
for estimate in ax:
    ...
    for mooring in [nrl4, nrl5]:
        ...
        he.append(ax[estimate].errorbar(
            ...
            xerr=(np.log10(shear['mean'] + shear['std'].values/np.sqrt(window * 24))
                  - np.log10(shear['mean'])),
            yerr=(np.log10(epsr['std'].values / np.sqrt(window * 24))
                  - np.log10(epsr['mean'])),
            fmt='o', ms=3))
        ...
        if estimate == 'niw':
            ...
            ax[estimate].plot(np.log10(shear['mean'][idx]),
                              np.log10(epsr['mean'][idx]), 'wo',
                              ms=2, zorder=10)
        ...
        ax[estimate].set_xlabel(r"log (mean squared shear [$s^{-2}$])")
    ...
    x_expanded = np.append(x_expanded, 0.98*np.nanmax(x[mask]))
    hfit = ax[estimate].plot(x_expanded, p[0]*x_expanded+p[1],
                             'k--', lw=2)

axx[0].set_ylabel(r'log $ε$ [W/kg]')
axx[0].set_ylim([-12, -5])
# axx[0].set_xlim([1e-12, 1e-7])
ax['niw'].legend(handles=he[len(labels):]+hfit,
                 labels=labels+['linear fit'],
                 loc='lower right')
...
f = plt.figure(constrained_layout=True)
f.set_constrained_layout_pads(h_pad=2/72, wspace=0.1)
f.set_size_inches(7, 6.7)
gs = f.add_gridspec(4, 1, height_ratios=[1, 1, 1, 0.5])
ax = list()
with mpl.style.context({'axes.facecolor': (0, 0, 0, 0.03),
                        'axes.grid': True,
...
hh = ax[1].scatter(time.values[mask].ravel()[isort],
                   ...
                   ,**kwargs)
ax[1].set_ylim((36, 33))
ax[1].set_yticks([33, 34, 34.5, 35, 36])
ax[1].set_ylabel('$S$')
...
ax[-2].set_yticklabels(['34.25', '34.75', '35.5'])
