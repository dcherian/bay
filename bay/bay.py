...
pods = {
    "RAMA12": {
        ...
        "ha": "right",
        "va": "center",
        "depths": {"15 m": "2014-15", "30 m": "2014-15", "45 m": "2015"},
    },
    "RAMA15": {
        "lon": 90,
        "lat": 15,
    ...
    "NRL1": {
        ...
        "ha": "center",
        "va": "top",
        "depths": {"60 m (55-100)": "2014", "80 m (75-115)": "2014"},
    },
    "NRL3": {
        "lon": 85.5,
        "lat": 8,
        ...
        "ha": "right",
        "va": "top",
        "depths": {"32 m (28-78)": "2014", "52 m (48-100)": "2014"},
    },
    "NRL4": {
        "lon": 87,
        "lat": 8,
        ...
        "ha": "center",
        "va": "bottom",
        "depths": {"63 m (60-85)": "2014", "83 m (80-105)": "2014"},
    },
    "NRL5": {
        "lon": 88.5,
        "lat": 8,
...
def make_merged_nc(moorings, fileprefix="bay_merged"):
    ...
    for m in tqdm.tqdm(moorings):
        ...
        Turb = xr.merge(
            [Turb, (subset.expand_dims(["lat", "lon"]).set_coords(["lat", "lon"]))]
        )
    ...
    Turb = Turb.rename({"ε": "epsilon", "χ": "chi-t"})
    ...
    (
        Turb.resample(time="1H", keep_attrs=True)
        .mean(dim="time")
        .to_netcdf(fileprefix + "_hourly.nc")
    )
    (
        Turb.resample(time="6H", keep_attrs=True)
        .mean(dim="time")
...
def nc_to_binned_df(
    ...
    df = turb.to_dataframe().dropna(subset=["KT"]).reset_index()

    latlon = (
        df["lat"].astype("float32").astype("str")
        + "N, "
    ...
    df["moor"] = latlon.map(dict(zip(moornames.values(), moornames.keys()))).astype(
        "category"
    )
...
def bin_ml_bl_rho(df, bins):
    ...
    mask_bl = np.logical_and(np.logical_not(mask_ml), depth_minus_ild <= error_depth)

    mask_ml_plus = np.logical_and(
        np.logical_not(np.logical_or(mask_ml, mask_bl)), depth_minus_mld <= 15
    )
    mask_ml_plus = np.zeros_like(mask_ml).astype("bool")
    mask_deep = np.logical_not(
        np.logical_or(np.logical_or(mask_ml, mask_bl), mask_ml_plus)
    )

    nrl3_ml = np.logical_and(df["moor"] == "NRL3", df["N2"] < 3e-6)
    nrl3_bl = np.logical_and(
        np.logical_and(df["moor"] == "NRL3", df["Tz"] < 2e-3), df["N2"] > 3e-6
    )
    ...
    df.loc[mask_deep, "bin"] = pd.cut(df.ρ.loc[mask_deep], bins, precision=1)
...
def calc_wind_input(kind="merra2", dask=False):
    ...
    if kind == "ncep":
        ...
        merra = xr.open_mfdataset(
            "/home/deepak/bay/datasets/merra2/*.nc", parallel=dask
        ).sel(trange)
        ...
        tau = (
            ...
            .compute()
        )  # needed because we take FFT along time

    mimoc = dcpy.oceans.read_mimoc()
    mld = mimoc.DEPTH_MIXED_LAYER.sel(**region).load()
    ...
    wind_input.attrs["description"] = (
        "Near-inertial power input calculated using "
        + windstr
    ...
    wind_input.to_netcdf("~/bay/estimates/" + windshortstr + "-wind-power-input.nc")
...
def bin_and_to_dataframe(KTm, ρbins=None, Sbins=None):
    ...
    KTdf["latlon"] = (
        KTdf["lat"].astype("str") + "N, " + KTdf["lon"].astype("str") + "E"
    ).astype("category")
    ...
    KTdf["moor"] = (
        KTdf["latlon"]
        .map(dict(zip(moornames.values(), moornames.keys())))
        .astype("category")
    )
    return KTdf
