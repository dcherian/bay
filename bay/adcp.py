...
def smooth_shear(var, n):
    ...
    var = (
        var.rolling(depth=n, center=True).construct("window_dim", stride=1).dot(weights)
    )
...
def to_isothermal_space(data, varnames, isotherms):
    ...
    isoT["mean_depth"] = xr.DataArray(
        isotherms.depth.values,
        dims=["T"],
        attrs={"long_name": "mean isothermal depth", "units": "m"},
    )
    isoT = isoT.set_coords("mean_depth")

    isoT["depth"] = (
        xr.DataArray(
            np.zeros((len(isos), len(data.time))),
            dims=["T", "time"],
            coords={"T": isos, "time": data.time},
        )
        * np.nan
    )
    ...
    for tt, t in tqdm.tqdm(enumerate(data.T.time.values)):
        for var in varnames:
            mask = ~np.isnan(data.T[:, tt].values) & ~np.isnan(data[var][:, tt].values)
...
def backrotate(ds, f0):
    ...
    tvec = xr.DataArray(
        (ds.time - ds.time[0]).values.astype("timedelta64[D]").astype("float32"),
        dims=["time"],
        coords={"time": ds.time},
    )
...
def process_adcp(mooring, argo_superset=None, nsmooth_shear=4, isothermal=False):
    ...
    if mooring.name == "NRL4":
        vel = mooring.vel[["u", "v"]].sel(time=slice(None, "2015-03-01")).copy()
    else:
    ...
    N2 = (
        -9.81
        * dcpy.eos.alpha(34, mooring.ctd.T, mooring.ctd.depth2)
        * mooring.ctd.T.differentiate("depth2").rename({"depth2": "depth"})
    )

    vel = (
        vel.interpolate_na("depth")
        .interpolate_na("time")
    ...
    vel["T"] = (
        mooring.ctd.T.rename({"depth2": "depth"})
    ...
    vel["wkbz"] = xr.DataArray(
        wkbz,
        dims=["wkbz"],
        coords={"wkbz": wkbz},
        attrs={"long_name": "WKB scaled depth", "units": "m"},
    )

    vel["wkbu"] = xr.DataArray(
        (vel.u / np.sqrt(vel.wkb_factor)).values,
        dims=["wkbz", "time"],
        attrs={"long_name": "WKB u", "units": "m/s"},
    )
    vel["wkbv"] = xr.DataArray(
        (vel.v / np.sqrt(vel.wkb_factor)).values,
        dims=["wkbz", "time"],
    ...
    filtered.attrs["description"] = (
        "bandpass filtered in "
        + str(freq_factor)
        + "f_0 = "
        + str(filter_kwargs["freq"])
        + " cpd."
    )

    vel.to_netcdf("../estimates/ebob-adcp/" + mooring.name.lower() + "-vel.nc")
    filtered.to_netcdf(
        "../estimates/ebob-adcp/" + mooring.name.lower() + "-vel-niw-filtered.nc"
    )

    # isothermal frame
    if isothermal:
        isotherms = (
            mooring.ctd.T.mean("time")
            .rename({"depth2": "depth"})
        ...
        isoT = to_isothermal_space(
            vel, ["wkbu", "wkbv", "uz", "vz", "uz_back_real", "uz_back_imag"], isotherms
        )
        ...
        isoT.to_netcdf(
            "../estimates/ebob-adcp/" + mooring.name.lower() + "-vel-isoT.nc"
        )
...
def read_adcp(name):
    ...
    vel["vel"] = xr.open_dataset(
        "../estimates/ebob-adcp/" + name.lower() + "-vel.nc"
    ).load()
    vel["niw"] = xr.open_dataset(
        "../estimates/ebob-adcp/" + name.lower() + "-vel-niw-filtered.nc"
    ).load()
    vel["iso"] = xr.open_dataset(
        "../estimates/ebob-adcp/" + name.lower() + "-vel-isoT.nc"
    ).load()

    # backrotation
    for var in ["vel", "iso"]:
        vel[var]["uz_back"] = vel[var].uz_back_real + 1j * vel[var].uz_back_imag
        vel[var] = vel[var].drop(["uz_back_real", "uz_back_imag"])

    vel["low"] = vel["vel"][
        ["u", "v", "uz", "vz", "KE", "wkbKE", "wkbu", "wkbv"]
    ].apply(xfilter.lowpass, coord="time", freq=0.1, cycles_per="D", order=3)
...
def partition_niw_up_down(data):
    ...
    dim = list(data.dims)
    dim.remove("time")
    fft = xrft.dft(data, dim=dim, shift=False, window=False, detrend="constant")
    ...
    up = xr.DataArray(
        np.fft.ifft(fft * upkernel, axis=axis),
        dims=data.dims,
        coords=data.coords,
    ...
    down = xr.DataArray(
        np.fft.ifft(fft * downkernel, axis=axis),
        dims=data.dims,
        coords=data.coords,
