...
full["u"] = uzi.u
full["v"] = uzi.v
full["N2"] = (mooring.turb.N2).isel(depth=1).interp(time=uzi.time)
full["Tz"] = mooring.turb.Tz.isel(depth=1).interp(time=uzi.time)
full = full.interpolate_na("time")
full.attrs['name'] = 'Full'

low = full.shear.pipe(xfilter.lowpass, freq=0.15, **filter_kwargs)
low.attrs['name'] = 'LF (< 6.6d)'
high = full.shear.pipe(xfilter.bandpass, freq=[0.15, 4], **filter_kwargs)
high.attrs['name'] = 'HF (> 6.6d)'
fM2 = full.shear.pipe(xfilter.bandpass,
                      freq=[0.95*(1.93-mooring.inertial.values),
                            1.05*(1.93 + mooring.inertial.values)],
                      ,**filter_kwargs)
niw = full.shear.pipe(xfilter.bandpass, freq=[0.15, 0.5], **filter_kwargs) + fM2
niw.attrs['name'] = 'NIW (6.6d - 2d) + fM2'
loni = full.shear.pipe(xfilter.bandpass, freq=[0.5, 4], **filter_kwargs) - fM2
...
def _plot(ax, e, s, color, timescale=timescale):
    ...
    s = (s.resample(time=timescale).apply(dcpy.util.rms)
         .where(count/24/30 > 0.7)
         .interp(time=KTr.time))
