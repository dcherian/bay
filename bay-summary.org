...
def compare_fluxes(mooring):
    ...
    wind_input = (xr.open_dataset(
        '/home/deepak/bay/estimates/wind-power-input-2014.nc')
    ...
    ax[1, 1].legend(['unfiltered flux', 'true flux'])
    ...
    f.suptitle(mooring.name + ' | NCEP wind power input comparison | data vs model.')
...
for ss, seas in enumerate(['NE', '2014-03', '2014-04', 'SW', 'SWNE']):
    ...
    if '2014' in seas:
        hniw = ((wind_input.wind_input*1000).sel(time=seas).mean('time')
                .plot(cmap=mpl.cm.GnBu, edgecolors='w', lw=0.5,
                      levels=np.arange(0, 0.9, 0.05), add_colorbar=False,
                      ax=ax[seas]))
...
f.colorbar(hniw, ax=list(ax.values()), orientation='horizontal',
           label='near-inertial energy flux $Π$ [mW/m²]', shrink=0.6)
...
f.savefig('images/wind-input-map-ts.png', bbox_inches='tight')
#+END_SRC
#+CAPTION: Split NESW to Mar, Apr so we can see that the Northern Bay is a little different. The southern Bay really sees winds die down but there are steady up-coast winds north of 13N (I've checked this with tropflux)
[[file:images/wind-input-map-ts.png]]
...
KT = (nrl5.KT.isel(depth=0)
      .resample(time='4D').mean('time').shift(time=1)
      .dropna('time'))
τ = np.sqrt(nrl5.tropflux.tau).resample(time='4D').mean('time').interp_like(KT)
ax[1].plot(τ, np.log10(KT), 'o', alpha=0.2, ms=4, color='C1')
...
** M_2 soliton mixing?
#+BEGIN_SRC ipython :session :results none
trange = slice('2014-06-01', '2014-10-01')
...
def _cd_plot(var, period, ax, label):
    ...
    amp = demod.amp.pipe(dcpy.ts.xfilter, flen=6*86400, dim='time', kind='hann')
...
for m in [nrl3, nrl4, nrl5]:
    ...
    KE = ((np.abs(m.vel.w.sel(depth=slice(120, 200)))**2)
          .mean('depth'))

    if 'shear_fit' not in m.vel:
        u = (m.vel.u.sel(depth=slice(120, 200))
             .interpolate_na('time')
             .dropna('time', how='any'))
        v = (m.vel.u.sel(depth=slice(120, 200))
             .interpolate_na('time')
             .dropna('time', how='any'))
...
[aa.set_xlabel('') for aa in ax]
ax[0].legend()
ax[0].set_title('8N NIW KE in thermocline (120-200m) | complex demod amplitude')
ax[1].set_title('8N NIW shear in thermocline (120-200m) | complex demod amplitude')
...
#+BEGIN_SRC ipython :session :ipyfile images/temp/imgkmuAvt.png
shear = nrl5.vel.shear.sel(depth=120, method='nearest').interpolate_na('time')
...
(S2.resample(time='H').mean().sel(time='2014')
 .plot(label='$S²$', ax=ax[1]))
ax[1].legend()
ax[1].set_ylabel('')

((Ri.where(Ri < 2).resample(time='D').count()/24)
 .plot(ax=ax[2], label='< 2'))
...
if ax.shape[0] > ax.shape[1]:
    ...
    axes['atm'] = dict(zip(bay.seasons, ax[:, 1]))
else:
    axes['oce'] = dict(zip(bay.seasons, ax[0, :]))
    axes['atm'] = dict(zip(bay.seasons, ax[1, :]))
...
for season in bay.seasons:
    ...
    hniw = ((niw.wind_input*1000).sel(monsoon=season)
            .plot.contourf(ax=axes['atm'][season], cmap=mpl.cm.Blues,
                           levels=40, vmin=0, vmax=0.8, add_colorbar=False))

    axes['atm'][season].quiver(tropflux.longitude, tropflux.latitude,
                               tropflux.taux.sel(monsoon=season),
                               tropflux.tauy.sel(monsoon=season),
                               scale=0.8, width=0.006)
...
hcb1 = plt.colorbar(heke, ax=ax[0, :2], orientation='horizontal',
                    label='EKE [m²/s²]', shrink=1)
hcb2 = plt.colorbar(hniw, ax=ax[0, 2:], orientation='horizontal',
                    label='Near-inertial wind input [mW/m²]',
                    shrink=1)
hcb1.set_ticks(np.arange(0, 0.31, 0.05))
hcb2.set_ticks(np.arange(0, 0.81, 0.2))

f.set_size_inches(6, 5)
plt.subplots_adjust(left=0.080, right=0.99, bottom=0.1, wspace=0.05, hspace=0.4, top=0.99)
...
#+END_SRC
*** Figure 3: RAMA vs NRL time series comparison
...
def plot_moor(moor, idepth, axx, time_range='2014'):
    ...
    (moor.KT.sel(time=time_range).isel(depth=idepth)
     .groupby(moor.KT.sel(time=time_range).time.dt.floor('D'))
     .count()/144).plot(ax=axes['coverage'], color='gray')
    ...
    axes['jq'].set_ylim([-50, 50])
    axes['js'].set_ylim([-1e-2, 2e-1])
    axes['jq'].set_ylabel('$J_q^t$ [W/m²]')
    axes['js'].set_ylabel('$J_s^t$ \n [$10^{-2}$ g/m²/s]')
    [set_ax_color(axes[aa], 'gray', 'right') for aa in ['js', 'coverage']]
...
axnrl5 = plot_moor(mooring, idepth=1, axx=axx5)
...
set_ax_color(axnrl5['depth'], 'gray', spine='right')

axnrl5['input'] = axnrl5['met'].twinx()
axnrl5['input'].plot(mooring.niw.time, mooring.niw.true_flux*1000, color='gray')
axnrl5['input'].set_ylabel('Near-inertial input\n$Π$[mW/m²??]')
set_ax_color(axnrl5['input'], 'gray', spine='right')
...
*** Figure 4: Vertical profile of K_T distributions
*** Figure 5: Vertical profiles of KT, Jq, etc. for 8N
#+BEGIN_SRC ipython :session :results none
...
# ra12.CombineTurb()  # reset

mm1 = ra12.χpod[526].convert_to_xarray('mm1')
tzm = mm1.dTdz.interp(time=ra12.Jq.time)
