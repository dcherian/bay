...
* cyclone daye
#+BEGIN_SRC ipython :session
subset = ((ostia.analysed_sst-273)
          .sel(**bay.region, time=slice('2018-09-12', '2018-09-27'))
          .isel(time=slice(None,None,2)))

fg = (subset-subset.isel(time=0)).plot(col='time', col_wrap=4, robust=True)

fg.map(lambda: plt.plot(90, 15, 'o', color='k'))

f.savefig('images/daye-sst.png')

aviso = xr.open_dataset('aviso-ssh-2018-nrt.nc')
#+END_SRC
#+CAPTION:
[[file:images/daye-sst.png]]
