...
# argo = dcpy.oceans.argo_mld_clim()
# argomld = (argo.mld_da_mean
#            .rename({'month': 'time'})
#            .sel(**bay.region)
#            .interpolate_na('lat'))
# argomld['time'] = pd.date_range('2014-01-01', '2014-12-31', freq='SM')[::2]
...
#+BEGIN_SRC ipython :session :ipyfile images/temp/imgWlSWCu.png

_, ax = plt.subplots(1,1,subplot_kw=dict(projection=ccrs.PlateCarree()))

bay.plot_coastline(ax)
bay.mark_moors(ax=ax, color='#eeeeee', colortext='#000aaa')

#+END_SRC

#+BEGIN_SRC ipython :session :ipyfile images/temp/imgWyIJdP.png
f, ax = plt.subplots(2, 1)

h0 = ax[0].plot([1,2], [3,5])
h1 = ax[1].plot([1,2], [3,8])
...
axes['NE'].set_ylim([16, 32])
axes['NE'].set_xlim([31, 36])
...
hcbar = f.colorbar(handles['ts'], ax=ax, extend=cmap_params['extend'],
