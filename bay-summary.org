...
plt.gcf().savefig('images/nrl5-isoT-shear-spectra.png')
#+END_SRC

#+CAPTION: isothermal shear spectra for 21C isotherm ≈ 130-150m. I still see the -f±M_2 peaks :/ What does this mean? Maybe doing this on isopycnals will fix this.
...
todo [5/5]
- [X] bring back net heat flux
- [X] color axes by variable
- [X] combine turbulent fluxes for each location in one panel so that I can see something
  - [X] again color axes
  - [X] rearrange for some symmetry
- [X] move 8N forcing down below
- [X] add total shear variance to last panel
