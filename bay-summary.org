...
trueflx = input_file.where(input_file.lat < 10, drop=True).true_flux
...
def avg_shear(ds, zrange):
    return (np.hypot(ds.uz, ds.vz)
            .sel(depth=zrange).mean('depth'))

def plot_shear_KT_avg(m, time_interval='2W', zrange=slice(30, 120)):
    ...
    lowpass = (adcp['vel'][['uz', 'vz', 'u', 'v']]
               .apply(xfilter.lowpass, coord='time',
                      freq=0.8 * f0, cycles_per='D', order=3))
    ...
    avgloshear = avg_shear(lowpass, zrange)
    avgnishear = avg_shear(adcp['niw'], zrange)
    avgshear = avg_shear(adcp['vel'], zrange)

    KT = (turb.KT.where(turb.KT < 0.1)
          .sel(**bay.loc[m]).mean(['depth'])
          .dropna('time'))

    tlim = KT.time[[0, -1]].values
    KTr = KT.resample(time=time_interval)
    ...
    (avgloshear
     .resample(time=time_interval).mean('time')
    ...
    ((avgloshear + avgnishear)
     .resample(time=time_interval).mean('time')
     .plot(ax=ax2, color='C2', yscale='linear', ls='--'))

    # axx[2].semilogy(avgshear.resample(time=time_interval)
    #                 .mean('time').squeeze()
    #                 .interp(time=KTr.median('time').time),
    #                 KTr.median('time'), 'o', color='C2')

    scatterx = ((avgloshear)
                .resample(time=time_interval)
                .mean('time').squeeze()
                .interp(time=KTr.median('time').time))
    ...
    axx[2].loglog(scatterx, scattery, 'o', color='C4')
    axx[2].grid()
    axx[2].text(0.05, 0.8, 'C² = '+str('{0:.2f}'.format(corrcoef**2)),
                transform=axx[2].transAxes)
    ...
    axx[0].set_title(m.upper())
    axx[1].set_xlim(tlim)

    f.savefig('images/kt-shear-scatter-' + m + '.png')


plot_shear_KT_avg('nrl4', time_interval='2W', zrange=slice(50, 120))
plot_shear_KT_avg('nrl5', time_interval='2W', zrange=slice(60, 120))
...
2. NRL4: Jun15 - Jul15 is outlier: there's shear but not much mixing
3. NRL5: Oct 13 - Nov 13 is again outlier: shear but not much mixing
*** NRL5
**** Wind stress
...
* EBoB near-inertial wave climate
** Processing Notes
...
4. Separate into up, down components by calculating FFT along depth at each time; use +ve wavenumber = up, -ve wavenumber=down; set appropriate region to zero and then ifft.
...
3. NRL5 is worse because there is a missing temperature depth for some reason. Need to look that up.
...
seasonal = (filtered
            .groupby(filtered.time.monsoon.labels)
            .mean('time'))
...
f, ax = plt.subplots(1, 2, constrained_layout=True, sharey=True, sharex=True)
line_kwargs = dict(hue='monsoon', yincrease=False, add_legend=False)
[aa.set_prop_cycle(bay.monsoon_cycler) for aa in ax]
seasonal.wkbKE.plot.line(ax=ax[0], y='wkbz', **line_kwargs)
hdl = seasonal.KE.plot.line(ax=ax[1], y='depth', **line_kwargs)
...
f.savefig('images/seasonal-mean-KE-profiles-nrl4.png')
...
def plot_seasonal_spectrum(w, nsmooth=5):
    ...
    ax_int = dict(zip(bay.seasons, axx[:, 1]))
    ...
    for season, grp in grouped:
        ...
        spec = xrft.power_spectrum(grp.sel(wkbz=slice(50, None)),
                                   dim=None, detrend='constant',
                                   window=True)
    ...
    vmin = 0.05
    vmax = 10
    levels = np.logspace(np.log10(vmax), np.log10(100), 5)

    f0 = w.inertial.values
    for season, grp in grouped:
        hdl = plot_2dspectrum(shear_spec[season], vmin=vmin, vmax=vmax,
                              norm=mpl.colors.LogNorm(),
                              ax=ax[season], add_colorbar=False)

        niw_band = np.logical_and(shear_spec[season].freq_time < -0.8 * f0,
                                  shear_spec[season].freq_time > -1.2 * f0)

        intspec = xrsp.integrate.trapz(shear_spec[season]
                                       .where(niw_band, drop=True),
                                       'freq_time')
        intspec.plot(y='freq_wkbz', xscale='linear', ax=ax_int[season])

        vmin, vmax = hdl.get_clim()

        (shear_spec[season]
         .plot.contour(levels=levels, colors='k', ax=ax[season]))

        ax[season].text(0.05, 0.8, season, color='w',
                        transform=ax[season].transAxes)

    f.colorbar(hdl, ax=axx[:, 0], orientation='horizontal', extend='both',
               label=w.attrs['name'].upper() + ' WKB shear spectral density'
               + ' $S(m, ω)$')
    [aa.set_xlabel('') for aa in axx.flat[:-1]]
    [aa.set_title('') for aa in axx.flat]
    [aa.set_ylabel('') for aa in list(ax_int.values())]
    ax_int['SWNE'].set_xlabel('near-inertial\nshear PSD\n $\int S(m,ω) \, dω$')

    f.savefig('images/seasonal-shear-2d-spectra-' + w.attrs['name'].lower()
              + '.png')
...
**** Up- & down-filtered data with mixing
...
def partition_niw_up_down(data):
    ...
    up = xr.DataArray(np.fft.ifft(fft * upkernel, axis=axis),
                      ...
                      attrs={'long_name': 'up ' + data.attrs['name']},
                      name='up')

    down = xr.DataArray(np.fft.ifft(fft * downkernel, axis=axis),
                        dims=data.dims, coords=data.coords,
                        attrs={'long_name': 'down ' + data.attrs['name']},
                        name='down')
    ...
    return xr.merge([up, down])


for mooring in ['nrl1', 'nrl2', 'nrl3', 'nrl4', 'nrl5']:
    adcp = bay.read_adcp(mooring)

    niw_shear = (niw.uz + 1j * niw.vz)
    niw_shear.attrs['name'] = 'NIW shear'

    # niw_vel = (niw.u + 1j * niw.v)
    # niw_vel.attrs['name'] = 'NIW velocity'

    updown = partition_niw_up_down(niw_shear
                                   .dropna('depth', how='all')
                                   .dropna('time', how='any'))


def plot_up_down(updown):
    f, ax = plt.subplots(4, 2, sharex='col', sharey='row',
                         constrained_layout=True,
                         gridspec_kw={'width_ratios': (4, 1),
                                      'height_ratios': (0.5, 1, 1, 0.75)})
    hdl = np.real(updown.down).plot(robust=True, ax=ax[2, 0], yincrease=False)
    clim = hdl.get_clim()
    np.real(updown.up).plot(robust=True, ax=ax[1, 0], vmin=clim[0], vmax=clim[1],
                            yincrease=False)

    nrl5.PlotχpodDepth(ax=ax[1, 0], color='k')
    nrl5.PlotχpodDepth(ax=ax[2, 0], color='k')

    nrl5.tropflux.tau.plot(ax=ax[0, 0])
    ax[0, 0].set_ylabel('τ [N/m²]')
    (nrl5.turb.KT.resample(time='D').mean('time')
     .plot.line(ax=ax[3, 0], x='time', yscale='log'))

    (((np.abs(updown.up)).mean('time'))
     .plot(ax=ax[1, 1], y='depth', yincrease=False))
    (((np.abs(updown.down)).mean('time'))
...
plot_up_down(updown)
plt.savefig('images/up-down-niw-shear-nrl5.png', bbox_inches='tight')
  #+END_SRC
#+CAPTION:
...
for m in ['nrl4', 'nrl5']:
    ...
    niw_ke.plot.contour(ax=ax['niw'], levels=[0, 0.005, 0.01, 0.03],
                        colors='b', yincrease=False)
    hdl = lowpass.Uz.plot(ax=ax['lo'], **kwargs)
    niw_shear.plot(ax=ax['niw'], **kwargs)

    # for aa in axx[:-1]:
    #     (adcp['vel'].zχpod.resample(time='D').mean('time')
    #      .plot.line(x='time', color='k', lw=2, ax=aa))

    (turb.KT.sel(**bay.loc[m.upper()]).dropna(dim='depth', how='all')
     .resample(time='6H').mean('time')
     .plot.line(x='time', ax=ax['turb'], yscale='log'))
...
hshear = (((niw_shear).rolling(time=3*24).var() * 1e5)
