...
KT = []
for m in [ra12, nrl1, nrl3, nrl4, nrl5]:
    ktsubset = m.KT.copy()
    ktsubset['season'] = ktsubset.time.monsoon.labels
    ktsubset['depth'].values = ktsubset.z.mean(dim='time')
...
KTm = xr.merge(KT)
KTdf = np.log10(KTm['KT']).to_dataframe().reset_index()
#+END_SRC
**** Make plot
...
bandwidth = 'scott'
kind = 'kde'
pal = sns.color_palette("GnBu_d", n_colors=13)
pal.reverse()
# pal = sns.cubehelix_palette(11, rot=-.25, light=0.7)

def label(x, color, label):
    ax = plt.gca()
    ax.text(0.8, 0.05, '{0:.0f}m'.format(float(label)),
...
with sns.axes_style(style="ticks",
    ...
    g = sns.FacetGrid(KTdf.dropna(), row="mean_depth", col="season",
                      col_order=['NE', 'NESW', 'SW', 'SWNE'],
                      hue="mean_depth", xlim=[-8, -0.5],
                      sharex=True, sharey=True, palette=pal,
                      aspect=4, size=1)

    if kind == 'kde':
        g.map(sns.kdeplot, "KT", clip_on=False, shade=True, alpha=0.6, lw=1.5, bw=bandwidth)
        g.map(sns.kdeplot, "KT", clip_on=False, color="w", lw=2, bw=bandwidth)

    if kind == 'dist':
        g.map(sns.distplot, "KT", hist=True, kde=True, rug=False,
              kde_kws={'lw': 2, 'cut': 0},
              hist_kws={'alpha': 0.8})
    ...
    g.fig.subplots_adjust(hspace=-0.48)
    g.set_xlabels('$K_T$')
    g.set_titles('')
    g.set(yticks=[])
