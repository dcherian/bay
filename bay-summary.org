...
handles = dict()
for name, label in zip(['KT', 'Tz'], ['c', 'g']):
    handles[name] = dict()
    handles[name]['r'] = (ra12.turb[name].isel(depth=0).sel(time=trange)
                          .resample(time='D').mean('time')
                          .plot(ax=axes[name], _labels=False, lw=1.2))
    handles[name]['n'] = (nrl5.turb[name].isel(depth=1).sel(time=trange)
                          .resample(time='D').mean('time')
                          .plot(ax=axes[name], _labels=False, lw=1.2))
    axes[name].text(x0, y0, label+') Daily avg. ' +
...
axes['shear'].text(x0, y0, 'i) Shear variance at 8N [$10^{-6}$ units]',
...
set_ax_color(axes['jq12'], hsr[0].get_color())
set_ax_color(axes['jq8'], hsn[0].get_color())
...
[nrl5.MarkSeasonsAndEvents(ax=axes[aa], events=False)
 for aa in ['jq0', 'js12', 'js8', 'KT', 'Tz', 'S', 'shear', 'input', 'coverage']]
