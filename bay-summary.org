...
#+BEGIN_SRC ipython :session
def calc_iso_surface(my_array, my_value, zs, interp_order=3, power_parameter=0.5):
    ''' '''
...
def calc_S0_depth(dataset, S0):
    isodepth = dataset.S.isel(depth=1).drop('depth').copy()
    isodepth.attrs['long_name'] = 'Depth of ' + str(S0) + ' isohaline'
    isodepth.attrs['units'] = 'm'
    for tt in range(4):
        isodepth.values[:, :, tt] = calc_iso_surface(dataset.S.values[:,:,:,tt],
                                                     S0,
                                                     zmat.values[:, :, :, tt])
...
_, zmat = xr.broadcast(nio.S, nio.depth)

base = argo

argo['iso'] = calc_S0_depth(argo, 34.75)
argo2014['iso'] = calc_S0_depth(argo2014, 34.75)
nio['iso'] = calc_S0_depth(nio, 34.75)
...
for mooring in [nrl3, nrl4, nrl5]:
    (mooring.ctd.depth.where(np.logical_and(mooring.ctd.S > S0-0.05,
                                            mooring.ctd.S < S0+0.05))
     .min('z')
     .plot(x='time', label=mooring.name))
...
#+END_SRC
#+CAPTION: Estimate depth of S=34.75 from data? This is nonsensical. I don't have upper water column coverage... so we're implicitly assuming that the 34.75 isohaline is beneath the first instrument always.
