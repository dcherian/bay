...
for func in [bay.read_ra12,
    ...
    flux.append(
        mooring.niw.true_flux.interp(
            time=pd.date_range(mooring.niw.time[0].values,
                               mooring.niw.time[-1].values,
                               freq='H')))


for ii, ff in enumerate(flux):
    if 'longitude' in ff.coords:
        flux[ii] = ff.rename({'longitude': 'lon', 'latitude': 'lat'})

    flux[ii] = ff.expand_dims(['lon', 'lat'])
    try:
        flux[ii] = ff.drop('depth')
    except ValueError:
        pass
...
if 'true_input' not in locals():
    ...
    true_input = (true_input.true_flux
