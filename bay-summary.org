...
KT = []
for m in [ra12, ra15, nrl1, nrl3, nrl4, nrl5]:
    ktsubset = m.KT.copy()
...
KTdf = KTm.to_dataframe().reset_index()
KTm['ρbins'] = ρbins
KTm['Sbins'] = Sbins
#+END_SRC
...
#+BEGIN_SRC ipython :session :results none
def vert_distrib(KTdf, category, kind='kde'):
    bandwidth = 'scott'
    ...
    with sns.axes_style(style="ticks",
        ...
        g = sns.FacetGrid(KTdf.dropna(), row=category, col="season",
                          col_order=['NE', 'NESW', 'SW', 'SWNE'],
                          hue=category, xlim=[-8, -0.5],
...
<<define-vert_distrib>>
vert_distrib(KTdf, 'mean_depth', 'kde')
#+END_SRC
