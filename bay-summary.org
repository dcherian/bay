...
ax, colors = bay.make_map(pods, DX=0.6, DY=0.55,
...
1. Internal waves: 1.2f < freq < 20 minutes
2. Near-inertial:
3. M2 : 0.9*f_M2 < freq < 1.1*f_M2

#+NAME: integrate_temperature_spectrum
#+BEGIN_SRC ipython :session :results none
kwargs = {'dt': 600/86400,
          'multitaper': True,
          'nsmooth': 5}
...
for moor in [nrl1, nrl2, nrl3, nrl4, nrl5]:
    print(moor.name)
    f0 = 1/(dcpy.oceans.coriolis(moor.lat)*86400)
    for season in E.index:
        T = moor.ctd['T'].sel(depth2=200, method='nearest')
        T = (T.where(T.monsoon.labels == season))
        ...
        Eiw[moor.name][season] = (
            integrate_spectrum(spec, 0.9*f0, 1/1200*86400))
        Eniw[moor.name][season] = (
            integrate_spectrum(spec, 0.9*f0, 1.2*f0))
        Em2[moor.name][season] = (integrate_spectrum(spec,
                                                     0.9*tides['M2'],
                                                     1.1*tides['M2']))
#+END_SRC
...
sns.heatmap(Eiw, ax=ax[0], robust=True, annot=True, linecolor='w', linewidths=3,
            cmap=mpl.cm.Reds, fmt='.2f')
ax[0].set_title('internal wave band')

sns.heatmap(Eniw, ax=ax[1], robust=True, annot=True, linecolor='w', linewidths=3,
            cmap=mpl.cm.Reds, fmt='.2f')
ax[1].set_title('NIW band')

sns.heatmap(Em2, ax=ax[2], robust=True, annot=True, linecolor='w', linewidths=3,
            cmap=mpl.cm.Reds, fmt='.2f')
...
:RESULTS:
# Out[119]:
[[file:images/temperature-seasonal-variance.png]]
:END:
