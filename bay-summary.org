...
* Map
#+BEGIN_SRC ipython :session :ipyfile images/map.png :results raw
...
ax.coastlines('10m', color='slategray', lw=0.75)
...
extract={'x': slice(80, 96), 'y': slice(2, 25)}
etopo.z.sel(**extract).plot.contour(
                        transform=ccrs.PlateCarree(),
                        add_colorbar=False,
                        levels=np.sort([-100, -500, -1000, -2000, -3000, -4000, -5000]),
                        colors=['gray'], linewidths=0.25)
ax.set_xlabel('')
ax.set_ylabel('')
...
for name in pods:
    ...
    labelargs['bbox']['facecolor'] = colors[pod['label']]

    ax.text(pod['lon']+dx, pod['lat']+dy, text,
            transform=ccrs.PlateCarree(),
            ha=pod['ha'], va=pod['va'],
            multialignment='left', **labelargs)


ax.text(90-0.35, 18, 'OMM/WHOI', fontsize=12, color=colors['OMM/WHOI'], ha='right', va='center')
ax.text(90, 13.5, 'RAMA', fontsize=12, color=colors['RAMA'], ha='left', va='center')
ax.text(87, 6.8, 'NRL', fontsize=12, color=colors['NRL'], ha='center', va='center')
...
def make_merged_KT(moorings):
    ...
    for m in moorings:
        ...
        ktsubset = ktsubset.expand_dims(['lat', 'lon'])
        ...
        _, seas = xr.broadcast(ktsubset.KT, ktsubset['season'])

        mean_depth = xr.zeros_like(ktsubset.KT)
    ...
    for m in moorings:
        merged = (KTm.sel(lon=m.lon, lat=m.lat).reset_coords().KT
                  .dropna(dim='depth', how='all')
                  .dropna(dim='time', how='all'))
        orig = m.KT.reset_coords().KT.dropna(dim='time', how='all')
        xr.testing.assert_equal(merged, orig)

    # KTm.to_netcdf('merged_KT_test.nc', format='netCDF4')
    KTm.resample(time='1H').mean(dim='time').to_netcdf('merged_KT_hourly.nc')
    KTm.resample(time='6H').mean(dim='time').to_netcdf('merged_KT_6hourly.nc')
...
def vert_distrib(KTdf, bins, varname='KT', pal=None, f=None, ax=None,
    ...
    plotkind = 'violin'
    base_marker_size = 5
    pal_dist = sns.color_palette("GnBu_d", n_colors=len(bins.unique())+2)
    pal_dist.reverse()
    ...
    if varname is 'KT':
        title = '$\log_{10}$ hourly averaged $K_T$'
    ...
    for index, (label, df) in enumerate(KTdf.groupby([bins, 'season'])):
        ...
        if plotkind is 'violin':
            ...
            hdl = ax[season].hist(var, bottom=zloc, density=True)
        ...
        # kind = df['moor'].map(map_kind)
        # ramadf = df[kind == 'RAMA']
        # nrldf = df[kind == 'NRL']
        # if not nrldf.empty and not ramadf.empty:
        #     rhdl = ax[season].violinplot(ramadf[varname], positions=[zloc],
        #                                  widths=[12],
        #                                  vert=False, showextrema=False)
        #     nhdl = ax[season].violinplot(nrldf[varname], positions=[zloc],
        #                                  widths=[12],
        #                                  vert=False, showextrema=False)
        #     trim_horiz_violin(rhdl)
        #     trim_horiz_violin(nhdl)
...
#+NAME: vert-profile
#+BEGIN_SRC ipython :session :ipyfile images/bay-KT-vert-profile-test.png :results drawer
<<define-vert_distrib>>
...
mask_deep = np.logical_not(np.logical_or(mask_ml, mask_bl))

KTdf['bin'] = ''
KTdf.bin[mask_ml] = 'ML'
KTdf.bin[mask_bl] = 'BL'
KTdf.bin[mask_deep] = pd.cut(KTdf.ρ[mask_deep],
                             get_kmeans_bins(10, KTdf['ρ'][mask_deep]),
                             precision=1)
...
# bins = pd.cut(KTdf.ρ, get_kmeans_bins(13, KTdf.ρ))

f, ax = vert_distrib(KTdf, KTdf.bin)
