...
def smooth_shear(var, n):
    ...
    var = (var.rolling(depth=5, center=True)
...
if 'argo_superset' not in locals():
...
vel = mooring.vel.copy()
vel['u'] = vel.u.interpolate_na('depth')
vel['v'] = vel.v.interpolate_na('depth')
vel['uz'] = smooth_shear(vel.uz, 5)
vel['vz'] = smooth_shear(vel.vz, 5)
...
vel['Nmean'] = argo.N.interp(pres=vel.depth.values).rename({'pres': 'depth'})
...
isoT['depth'] = (xr.DataArray(np.zeros((len(isos), len(vel.time))),
                              dims=['T', 'time'],
                              coords={'T': isos,
                                      'time': vel.time})
                 ,* np.nan)
isoT['wkbu'] = xr.zeros_like(isoT.depth) * np.nan
isoT['wkbv'] = xr.zeros_like(isoT.depth) * np.nan

for tt, t in tqdm.tqdm(enumerate(vel.T.time.values)):
    for var in ['wkbu', 'wkbv', 'uz', 'vz']:
        mask = ~np.isnan(vel.T[:, tt].values) & ~np.isnan(vel[var][:, tt].values)

        if not np.any(mask):
            continue

        isoT[var][:, tt] = np.flip(np.interp(
            np.flip(isos), np.flip(vel.T.values[mask, tt]),
            np.flip(vel[var].values[mask, tt]),
            left=np.nan, right=np.nan))

filtered = vel[['u', 'v', 'wkbu', 'wkbv', 'uz', 'vz']].apply(
    xfilter.bandpass, coord='time', freq=[0.8, 1.2]*mooring.inertial.values,
    cycles_per='D')

tvec = xr.DataArray((filtered.time - filtered.time[0]).values
                    .astype('timedelta64[D]').astype('float32'),
                    dims=['time'], coords={'time': filtered.time})
filtered['uz_back'] = ((filtered.uz + 1j * filtered.vz)
                       ,* np.exp(1j * 2 * np.pi * mooring.inertial.values * tvec))
filtered['uz_back'].attrs['long_name'] = 'Backrotated inertial shear'
filtered['uz_back'].attrs['units'] = '1/s'

iso_filtered = (isoT[['wkbu', 'wkbv']]
                .interpolate_na('time', limit=12)
                .apply(xfilter.bandpass, coord='time',
                       freq=[0.8, 1.2]*nrl4.inertial.values,
                       cycles_per='D'))

filtered = filtered.dropna('depth', how='all').sel(time=slice('2013-12', '2014-11'))
iso_filtered = iso_filtered.dropna('T', how='all').sel(time=slice('2013-12', '2014-11'))
...
iso_kwargs = dict(y='mean_depth', levels=20)
  #+END_SRC
#+CAPTION:
[[file:images/nrl5-near-inertial.png]]
