...
for season in bay.seasons:
    ...
    axes['atm'][season].quiver(niw.lon, niw.lat,
                               niw.taux.sel(monsoon=season),
                               niw.tauy.sel(monsoon=season))
...
f, axx = plt.subplots(8, 1, sharex=True, constrained_layout=True)
f.set_constrained_layout_pads(hspace=0.001, h_pad=0)
axes = dict(zip(['met12', 'met8', 'KT', 'Jq', 'Js', 'Tz', 'S', 'shear'], axx))

hmet12 = (ra12.met.τ.resample(time='D').mean()
          .sel(time='2014').plot(ax=axes['met12'], color='k', lw=1.2))
...
nrl5.tropflux.tau.sel(time='2014').plot(ax=axes['met8'], color='k', lw=1.2)
...
handles = dict()
for name in ['KT', 'Tz', 'Jq', 'Js']:
    handles[name] = dict()
    handles[name]['r'] = (ra12.turb[name].isel(depth=0).sel(time='2014')
                          .resample(time='D').mean('time')
                          .plot(ax=axes[name], _labels=False, lw=1.2))
    handles[name]['n'] = (nrl5.turb[name].isel(depth=1).sel(time='2014')
                          .resample(time='D').mean('time')
                          .plot(ax=axes[name], _labels=False, lw=1.2))
    axes[name].text(x0, y0, 'Daily avg. ' +
                    xr.plot.utils.label_from_attrs(nrl5.turb[name]),
                    transform=axes[name].transAxes)

annotate_end(handles['KT']['r'][0], label='12N 15m')
...
hsr = (ra12.ctd.S.isel(depth=1).sel(time='2014')
       .resample(time='6H').mean(dim='time')
       .plot(ax=axes['S'], _labels=False))
hsn = (nrl5.ctd.S.isel(z=0).sel(time='2014')
       .resample(time='6H').mean(dim='time')
       .plot(ax=axes['S'], _labels=False))

_, htsr = annotate_end(hsr[0], label='12N 10m')
_, htsn = annotate_end(hsn[0], label='8N 105m')
htsr.set_va('top')

[aa.set_xlabel('') for aa in axx]
[aa.set_title('') for aa in axx]
[nrl5.MarkSeasonsAndEvents(ax=aa, events=False) for aa in axx]
...
axes['S'].text(x0, y0, 'Salinity [psu]', transform=axes['S'].transAxes)
axes['met8'].text(x0, y0, 'Forcing at 8N', transform=axes['met8'].transAxes)

axes['Jq'].set_zorder(10)

handles['Jq']['r'][0].set_clip_on(False)
handles['Jq']['r'][0].set_in_layout(False)
...
ebob = ebob.where(np.abs(ebob['Jq']) < 2000)

grouped = ebob.groupby(['season', 'bin'])
averaged = grouped.mean().to_xarray()
stddev = grouped.std().to_xarray()
...
# gs = f.add_gridspec(2, len(varnames), height_ratios=[20, 1])
# ax = list()
# ax.append(f.add_subplot(gs[:, 0]))
# for idx in range(1, len(varnames)):
#     ax.append(f.add_subplot(gs[:, idx], sharey=ax[0]))
...
for var in varnames:
    for ss in averaged.season:
        ...
        hdl = axes[var].errorbar(averaged.sel(season=ss)[var].values,
...
axes['KT'].set_ylim([110, 20])

# axes['leg'].legend(averaged.season.values, ncol=4, loc='center')

[axes[aa].axvline(0, color='gray', ls='--', zorder=-10)
 for aa in ['Jq', 'Js']]
axes['S'].axvline(35, color='gray', ls='--', zorder=-10)
axes['KT'].grid(True, axis='x')
axes['Js'].set_xlim([-0.001, 0.01])
