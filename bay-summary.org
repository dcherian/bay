...
def vert_distrib(KTdf, bins, varname='KT', pal=None, f=None, ax=None,
    ...
    for index, (label, df) in enumerate(KTdf.groupby([bins, 'season'])):
        ...
        for mm in df['moor'].unique():
            if len(df['moor'].ix[df['moor'] == mm])/len(df) <= 0.15:
                df = df[df['moor'] != mm]
...
#+NAME: vert-profile
#+BEGIN_SRC ipython :session :ipyfile images/bay-KT-vert-profile-new.png :results drawer
<<define-vert_distrib>>
...
if 'KTm' not in locals():
    ...
    ρbins = get_kmeans_bins(11, KTdf['ρ'])
...
:RESULTS:
[[file:images/bay-KT-vert-profile-new.png]]
:END:

1. High mixing at 70-10m depth in SW monsoon, compared to NE monsoon.
...
#+BEGIN_SRC ipython :session :ipyfile images/temp/img0WlFMK.png
...
1. Some of these are where T_z changes sign frequently. (╯°□°）╯︵ ┻━┻
...
ax = ra15.Plotχpods(region={'time': slice('2015-02-01', '2015-03-31')},
                    filt='hann',  filter_len=None, Tlim=[26.5,28])
...
When the freshwater arrives we see stronger M_2 tides, maybe a packet came by?
...
(ra15.ctd.S.sel(depth=[10, 20, 40, 60],
                time=slice('2015-02-01', '2015-03-31'))
...
dcpy.ts.PlotSpectrum(ra15.ctd.S.sel(depth=40,
                                    time=slice('2015-02-12', '2015-03-03')),
                     ,**specargs)
dcpy.ts.PlotSpectrum(ra15.ctd.S.sel(depth=40,
                                    time=slice('2015-03-03', '2015-03-30')),
                     ,**specargs)
...
plt.legend(['2015-Feb-12 - 2015-Mar-05', '2015-Mar-05 - 2015-Mar-30'])
