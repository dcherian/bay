...
g.map_dataarray(xr.plot.contourf, 'longitude', 'latitude',
                cmap=mpl.cm.OrRd,
...
for idx, aa in enumerate(ax.flat):
    aa.clabel(g._mappables[4+idx], fmt='%2.f', fontsize='smaller')
...
g.set_titles('')
[aa.set_xticks(range(78, 96, 4)) for aa in g.axes.flat]
[aa.set_yticks(range(2, 24, 4)) for aa in g.axes.flat]
