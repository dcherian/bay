...
kwargs = dict(dim='time', nfft=20*86400/10/60, shift=2*86400/10/60,
              multitaper=True)
...
# KE spectrum
f, axx = plt.subplots(4, 2, constrained_layout=True, sharex=True, sharey=True)
ax = dict()
ax['KE'] = dict(zip(bay.seasons, axx[:, 0]))
ax['ζ'] = dict(zip(bay.seasons, axx[:, 1]))

KEgroups = nrl5.vel.KE.groupby(nrl5.vel.time.monsoon.labels)
...
for ((label, kgrp), (_, zgrp)) in zip(KEgroups, ζgroups):
    ...
    g = gm81(N=N, f0=dcpy.oceans.coriolis(30), b=1300)
    gmKE = g.KE('ω')
...
axx[0, 0].set_xlim((0.8 * g.f0/2/np.pi, 1/600/2))
...
if 'wind_input' not in locals():
    ...
    niw = wind_input.groupby(wind_input.time.monsoon.splitlabels).mean('time')
...
for season in bay.splitseasons:
    ...
    hniw = ((niw.wind_input*1000).sel(monsoon=season)
            .plot.contourf(ax=axes['atm'][season], cmap=mpl.cm.GnBu,
                           levels=40, vmin=0, vmax=1.2, add_colorbar=False))

    axes['atm'][season].quiver(niw.lon[::2], niw.lat[::2],
                               niw.taux.sel(monsoon=season)[::2, ::2],
                               niw.tauy.sel(monsoon=season)[::2, ::2],
...
((1.414 * niw_shear * 1000).rolling(time=3*24).std()
...
*** Figure 6: Vertical profiles of KT, Jq, etc. for 8N
#+BEGIN_SRC ipython :session :results none
