...
ax = ra12.Plotχpods(quiv=False, filt='hann', filter_len=12*3600.0,
                    TSkind='timeseries', met='local')
...
#+NAME: ra12-2014-summary
#+BEGIN_SRC ipython :session :ipyfile images/ra12-2014-summary.png
%matplotlib inline
ax = ra12.Plotχpods(quiv=False, filt='mean', filter_len=1*86400.0,
...
#+RESULTS: ra12-2014-summary
:RESULTS:
[[file:images/ra12-2014-summary.png]]
:END:

#+NAME: ra12-2015-summary
...
ax = ra12.Plotχpods(quiv=False, filter_len=1*3600, filt='hann',
                    TSkind=TSkind,
                    region=dict(time=slice('2013-12-29', '2014-02-08')))
...
plt.gcf().savefig('images/ra12-FW1.png')
#+END_SRC
#+CAPTION: Irrawady fresh water pulse.
...
** 45m monthly signal?
#+BEGIN_SRC ipython :session :ipyfile images/temp/imgGGG6AO.png
f, ax = plt.subplots()

for zz in ra12.ε.depth:
    ts = np.log10(ra12.ε.sel(depth=zz, time='2015')
                  .resample(time='6H').mean(dim='time'))


    kwargs = dict(dt=6/24, preserve_area=False, lw=0.75, linearx=True)
    # dcpy.ts.PlotSpectrum(ts, decimate=False)

    dcpy.ts.PlotSpectrum(ts, multitaper=True, ax=ax, **kwargs)

dcpy.plots.linex([1/30, 1/20, 1/10, 1/5, 1/2, 1, 1.9])
dcpy.plots.linex(ra12.inertial, color='r')
#+END_SRC
...
** WDA estimates
#+BEGIN_SRC ipython :session :ipyfile images/temp/img6iDhwd.png
plt.style.use('seaborn-colorblind')

tslice = slice('2014-10-04', '2014-10-14');
tslice = slice('2014-01-27', '2014-02-08');

with plt.style.context({'font.size': 14}):
    jregion = {'Jtime': tslice}
    region = {'time': tslice, 'depth': 15}

    f, axx = plt.subplots(2, 1, sharex=True);

    ax = axx[0]

    ra12.met.Jq0.sel(**jregion).plot(ax=ax, label='Jq0')

    ra12.Jq.sel(**region).plot(ax=ax, label='Jqt WDA')
    ax.plot(ra12.χpod[526].chi['mm1']['time'], ra12.χpod[526].Jq['mm1'],
            label='Jqt mm1')
    ax.axhline(0, color='gray', zorder=-10)

    ax2 = ax.twinx()
    ax2.plot(ra12.χpod[526].chi['mm1']['time'],
             ra12.χpod[526].chi['mm1']['dTdz'], 'k', label='Tz moor')
    ra12.Tz.sel(**region).plot(ax=ax2, color='r', label='Tz WDA')
    ax2.set_ylim([-0.03, 0.03])
    ax2.axhline(0, color='k', zorder=-10)
    plt.gcf().legend()

    Lmo  = ra12.monin_obukhov()
    Lmo.plot(ax=axx[1])
    axx[1].set_ylim([-100, 30])
    dcpy.plots.liney([-15, 15, 0], ax=axx[1])
    ax.set_xlim([region['time'].start, region['time'].stop])
    ax.set_ylim([-400, 400])
...
ax = ra15.Plotχpods(quiv=False, filt='mean', filter_len=1*3600.0,
                    TSkind='pcolor', met='tropflux')
ax['Jq'].set_ylim([-100, 100])
# ax['S'].set_ylim([30, 34])
...
* further filtering
My daily means are small because I get large positive heat fluxes at night. I am going to filter saying that I don't know the sign when mooring |dTdz| < 4e-4 (using seabirds accuracy spec of 2e-3 C ≡ 2 instruments * 2e-3C / 10m).

#+BEGIN_SRC ipython :session :results none
mm1 = ra12.χpod[527].convert_to_xarray('mm1')
mmw = ra12.χpod[527].convert_to_xarray('mm1w')

mm1i = mm1.interp_like(mmw)

mmw_filt = mmw.where(~np.logical_and(mm1i.dTdz.rolling(time=6).mean() < 0,
                                     mm1i.dTdz > -1e-3))
...
mmw.Jq.resample(time='D').mean('time').plot()
mmw_filt.Jq.resample(time='D').mean('time').plot()
...
turb = ra12.turb.sel(time='2015')
turb['mld'] = turb.mld.rolling(time=24).mean()
mlturb = xr.Dataset()
for var in ['Jq', 'KT']:
    mlturb[var] = turb[var].sel(depth=15)
    mlturb[var] = xr.where(turb.mld > 25, turb[var].sel(depth=30), mlturb[var])
    mlturb[var] = xr.where(turb.mld > 39, turb[var].sel(depth=45), mlturb[var])
