...
A particularly strong set of near-inertial wave packets forced significantly enhanced turbulence at the χpod's depth during this time period, and coincided with a set of $M_2$ tide packets that vertically displaced isotherms and shear layers (Figure [[ref:fig:nrl5-niw]]b).
...
for season, aa in axes['big'].items():
    ...
    hdl = mean_precips.precipitation.sel(monsoon=season).plot.contourf(
        ax=aa, levels=11, cmap=dcpy.plots.rain_colormap(), add_labels=False,
        zorder=1, vmin=0, vmax=20, add_colorbar=False)
    ...
    aa.set_yticks(np.arange(-8,
                            big_region['latitude'].stop, 5))

    aa.set_xlabel('Longitude')
    aa.set_title('')
    aa.text(48, 20, season, fontsize=10)
    dcpy.plots.liney(0, ax=aa)
    [common(aa) for aa in axarr.ravel()]

f.colorbar(hdl, ax=axes['big']['SW'], label='Precipitation [mm/day]')
...
dcpy.plots.label_subplots(list(axes['big'].values()) + list(axarr.flat),
                          x=0.94, y=0.9,
                          ha='center',
                          fontsize=8,
...
f.set_size_inches(6.5, 8.2)
plt.tight_layout()
f.canvas.draw()
f.canvas.draw()

pos0 = axes['big']['NE'].get_position()
axes['big']['NE'].set_position([pos0.x0+0.1, pos0.y0, pos0.width, pos0.height])
...
bay.make_vert_distrib_plot('KT', '../estimates/bay_merged_mooring_hourly.nc',
                           ...
                           moor=moors,
                           adjust_fig=False, markers={'mean': 'd', 'median': 'v'},)
