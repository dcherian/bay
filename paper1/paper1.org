#+LATEX_CLASS: ametsoc
#+TITLE:  The seasonal cycle of upper-ocean mixing in the Bay of Bengal
#+OPTIONS: author:nil timestamp:t email:nil toc:nil num:2 title:nil H:3 p:nil tasks:todo tags:nil
#+PROPERTY: header-args :eval never-export :exports none :results drawer
...
\abstract{
We describe the seasonal cycle of mixing in the top \SI{100}{m} of the Bay of Bengal as observed by moored mixing meters (χpods) deployed throughout the Bay in 2014 and 2015.
...
The seasonal cycle of near-surface $K_T$ (top 45m) in the Bay appears to follow the seasonal cycle in wind forcing.
...
In the south-central Bay (8°N, 85°E--89°E), monthly-averaged turbulent transport of salt out of the salty Arabian Sea water between August and January is significant relative to local $E-P$.
...
Meanders in the Summer Monsoon Current forced by Rossby waves that trace their origin back to equatorial wind fluctuations make the southwestern and south-central Bay a site of strong mesoscale activity during the southwest monsoon (upper row of our Figure ref:fig:spatial and citealp:Vinayachandran1998).
...
Intense near-inertial input in the Bay is forced by the passage of cyclonic systems as in the mid-latitudes citep:Alford2003.
There is approximately no near-inertial energy flux into the mixed layer during March (northern Bay) and April (entire Bay).

Here we summarize year-long observations of turbulence at multiple sites in the Bay (Figure ref:fig:map) and find that the seasonal cycle of winds, currents and near-inertial energy is imprinted on both near-surface and thermocline turbulence (Section ref:sec:profile).
Correlations of estimated turbulence quantities with wind-stress and local shear, as well as implications of the observed mixing for salt budgets of the Bay are examined (Sections ref:sec:drivers and ref:sec:8n).
...
This process yields the turbulence dissipation rate of temperature variance $χ$ after relating $χ$ to $ε$, the turbulence dissipation rate of kinetic energy, by assuming $K_T = K_ρ = Γε/N^2$ i.e. that the turbulence diffusivities of temperature and density are equal, and that mixing efficiency $Γ = 0.2$ for stratified turbulence citep:Moum2009a.
...
*** The challenge of weak background gradients
...
Such behaviour is especially problematic in the Bay where salinity often dominates near-surface stratification and temperature inversions are common (e.g. citealp:Shroyer2016), unlike in the equatorial cold tongues where χpods have been most intensively deployed (e.g. citealp:Perlin2012 where $T_z \ge \SI{1e-2}{\celsius \per\m}$).
...
*** The challenge of weak turbulence

A second challenge with analyzing χpods deployed in the Bay's thermocline is the frequent occurrence of weakly turbulent and near-laminar flow for extended periods of time as has been recorded with microstructure measurements in the Aegean Sea citep:Gregg2012 and in the Arctic citep:Scheifele2018.
...
During such time periods, the χpod records ``bit noise'' and such behaviour is detectable using knowledge of the  circuit components involved (Appendix D).
When the recorded temperature variance is within an arbitrary factor of 1.5 of the inferred noise variance, we set $ε$ to NaN and $χ$ to 0 resulting in $K_T, K_S$  being set to molecular values $κ_T, κ_S$ and the resulting fluxes $J_q^t, J_s^t$ being that due to molecular diffusion (eqn. ref:eq:def).
...
1) The mixed and isothermal layer depths (MLD, ILD respectively) are defined using threshold criteria $Δρ > \SI{0.03}{\kg\per\m\cubed}$ and $ΔT > \SI{0.2}{\celsius}$ respectively. The mixed and barrier layer depths are only determinable at the RAMA moorings with CTDs at 1m, 10m, 20m, 40m, 60m and 100m. Measurements are within the mixed layer if the depth of the χpod $z_χ \le \text{MLD} + \SI{5}{m}$ and within the barrier layer if $\text{MLD} + \SI{5}{m} < z_χ \le \text{ILD} + \SI{5}{m}$[fn::NRL3 χpods appear to be within the mixed layer and barrier layers for a few weeks in February. There is only one CTD above these χpods, so we label NRL3 measurements as being within the mixed layer if $N^2 < \SI{5e-6}{\per\s\squared}$ and within the barrier layer if $T_z < \SI{2e-3}{\celsius\per\m}$ and $N^2 > \SI{5e-6}{\per\s\squared}$.].
2) We label every averaged $K_T$ measurement with the density value of the parcel as well as the depth of measurement. Mixed or barrier layer measurements are instead labelled with `ML' and `BL' respectively.
3) All measurements are then binned by `ML', `BL' or by density with bin edges [1018, 1021, 1022, 1022.5, 1023, 1023.5, 1024.25, 1029] \SI{}{kg/m^3}.
4) For each season, we construct a PDF of $K_T$ in each bin and calculate the mean and standard deviation of the depths of measurement.
5) The PDFs are presented at the mean depth of the density bin as a vertical profile (Figure ref:fig:vert). Each PDF is also labelled with either `ML', `BL', or the mean density in each bin as appropriate. The vertical bars indicate the standard deviation of depths of observation; these frequently overlap each other. Circles/diamonds and triangles indicate means and medians respectively. The horizontal line at the base of each PDF spans the range between the minimum observed value and the $99^{\text{th}}$ percentile. The PDFs are colored so as to indicate the amount of data in each bin (see caption).
...
Next we study the seasonal cycle in Figure ref:fig:vert in more detail and examine possible drivers of this seasonal cycle.
...
The seasonal cycle of thermocline turbulence is dramatic.
Near-molecular values of $K_T$ are observed during the NESW transition period --- mean $K_T \le \SI{1e-5}{\m\squared\per\s} (50κ_T)$ and median $K_T \le \SI{1e-6}{\m\squared\per\s} (5κ_T)$ for depths greater than \SI{50}{m} during March and April, assuming a typical value of molecular diffusivity $κ_T = \SI{2e-7}{\m\squared\per\s}$ (Figure ref:fig:vert).
During this period, the 105-m χpod at NRL5 observed mixing weaker by one to two orders of magnitude relative to the RAMA 12°N 15m χpod discussed previously (compare Figure [[fig:rama]]b and Figure [[fig:nrl]]b).
...
Shear can only be estimated at the deeper χpod on each NRL mooring because the shallower χpod was deployed within the blanking zone of the downward looking ADCP.
Shear is estimated by central differencing the velocity over three \SI{8}{m} wide bins that span that χpods depth level.
...
Variance-preserving /Eulerian/ rotary spectra of the WKB-scaled shear time-series at all NRL moorings were dominated by a broad peak at $-f_0$, narrow secondary peaks at $-f_0 \pm ω_{M2}$ ($ω_{M2}$ is the $M_2$ tidal frequency) and significant energy at frequencies $\le \SI{20}{days}$ reflecting meanders of the Summer Monsoon Current (primarily at NRL3, the easternmost mooring on the 8°N line).
The narrow peaks at $-f_0 \pm ω_{M2}$ are a sign of vertical pumping of near-inertial shear layers by the internal $M_2$ tide which Doppler-shifts some spectral energy from $-f_0$ to $-f_0 \pm ω_{M2}$ citep:Alford2001a.
In Figure [[ref:fig:nrl5-niw]] we present an example of such tidal pumping and resulting modulation of $K_T$ at the $M_2$ period between July 23 and August 7, when maximum mixing is observed at NRL5 (highlighted in white in Figure [[fig:nrl]]b,c).
A particularly strong set of near-inertial wave packets forced significantly enhanced turbulence at the χpod's depth during this time period, and coincided with a set of $M_2$ internal tide packets that vertically displaced isotherms and shear layers (Figure [[ref:fig:nrl5-niw]]b).
We interpret the apparent modulation of $K_T$ at near-$M_2$ frequency (Figure [[fig:nrl5-niw]]a) as a result of the $M_2$ internal tide heaving near-inertial shear layers past the χpod, and not mixing forced by tidal shear.

Guided by the spectrum, we use a second-order Butterworth filter applied forwards and backwards to split the WKB-scaled shear time series into two components: (a) low-frequency shear $S\low$ (lowpass with half power point 9 days) and (b) near-inertial shear $S\niw$ (bandpass between half power points 7 days and 2 days respectively).
The effect of tidal pumping could be substantially reduced by performing the filtering in isopycnal space (for e.g. citealp:Alford2017) and isopycnal rotary spectra do show substantially weakened secondary peaks at $-f_0 \pm ω_{M2}$ as in cite:Alford2001a.
...
A seasonal cycle is not evident in Richardson number $\Ri = N^2/u_z^2$ estimated using 20m-scale shear (not WKB scaled) and $N^2$ sampled hourly.
...
To aid interpretation, the 45° line is plotted in each panel: high $K_T$ values in the upper-triangle reflect co-occurrence of mixing with large $S^2\niw$ while high $K_T$ values in the lower-triangle reflect co-occurrence of mixing with large $S^2\low$ i.e. the Summer Monsoon Current.

*** NE monsoon

During the NE monsoon (Dec - Feb), mean values of $K_T \ge \SI{1e-5}{\m\squared\per\s} (50κ_T)$; medians are lower by one to two orders of magnitude (Figure ref:fig:vert).
...
Similar periods of low to negligible mixing are evident at other χpods --- /median/  $K_T \le \SI{1e-6}{\m\squared\per\s} \approxeq 5κ_T$ in most thermocline density bins (deeper distributions in Figure ref:fig:vert).
...
cite:Jinadasa2016 report vertical profiles of $N^2$ and turbulent kinetic energy dissipation rate $ε$ from which we infer minimum diffusivity at 87°E, 16°N, \SI{30}{m} (their Figure 2)
\begin{align}
  K_ρ^\text{min} &= \frac{Γε^\text{min}}{N^2} \approx \frac{0.2 \times 1 \times \SI{1e-9}{\W\per\kg}}{\SI{1e-3}{\per\s\squared}}  \\
                &\approx\SI{2e-7}{\m\squared\per\s} \approxeq κ_T,
\end{align}
...
cite:St.Laurent2017 too infer $K_ρ \approxeq \SI{1e-6}{\m\squared\per\s} (5κ_T)$ for depths between 40m and 120m by combining a mean vertical profile of $ε$ and mean $N$ collected by glider-based sensors over seven days.
Low thermocline diffusivities are predicted by the finestructure internal-wave scaling of cite:Henyey1986 and have been observed previously at low latitudes: $K_ρ \approx (1\mhyph 3) \times \SI{1e-6}{\m\squared\per\s} (5-15 κ_T)$ for latitudes south of 10°N in cite:Gregg2003.
...
Low values of $K_T$ are perhaps not surprising given the observations of cite:Jinadasa2016 and cite:Kunze2006 but these χpod observations are the first to show that extremely low mixing ($K_T \le 1\mhyph 10 κ_T$) persists for multiple weeks at multiple locations in the south-central Bay (Figure ref:fig:vert).
...
The seasonal cycle of near-surface turbulence follows from a seasonal cycle in direct wind stress forcing (Figure ref:fig:rama).
...
Climate model configurations that account for the latitudinal variation of diffusivity noted in cite:Gregg2003 such as cite:Jochum2009, CCSM4 citep:Danabasoglu2012 and cite:Chowdary2016a use a background $K_T \approx (1 \mhyph 1.7) \times \SI{1e-5}{\m\squared\per\s} (50 κ_T)$ in the Bay (citealp:Danabasoglu2012; their Figure 1).
...
Another puzzle is the extended period of low mixing during March and April — is the internal wave field weaker than that expected from the Garrett-Munk spectrum and if so, why?
...
* Appendix C: Estimating sorted gradients for surface-pumped χpods :ignore:
\appendix[C]
...
* Appendix D: Detecting weak turbulence :ignore:
\appendix[D]
...
We estimate the spectral energy level of the discretized white noise voltage time series of that amplitude and combine it with the instrument calibration coefficients as in cite:Becherer2017 to get a dimensional spectral energy density level that would result when the ADC records ``bit noise''.
Multiplying this noise spectral energy density level by frequency bandwidth gives an estimate of the instrument's ``noise floor'' i.e. an estimate of the variance when the data recorded is bit noise.
...
[[file:figure1-spatial-maps-label.pdf]]
...
#+NAME: fig:vert
#+ATTR_LATEX: :float multicolumn
#+CAPTION: \small The seasonal cycle of $K_T$. (top) Basin-wide seasonal averages of daily Tropflux wind stress $\tau$, near-inertial energy input $\Pi$, daily Tropflux net surface heat flux $J_q^0$ and TRMM precipitation $P$. Wind stres variability is represented by gray ellipses with widths and heights equal to twice the standard deviation of $\tau_x$ and $\tau_y$. (bottom) Vertical profile of hourly averaged $K_T$ formed by combining all estimates in density bins (Section [[sec:profile]]). PDFs as well as means and medians are shown. Bins are marked by 'ML' (mixed layer), 'BL' (barrier layer) and $\rho-1000$ for deeper bins. PDFs of both moored and sorted estimates are shown for the upper three bins (above and below the baseline respectively). Orange horizontal line marks the climatological depth of the $S=34.75$ isohaline at 8N estimated using the Argo climatology. Vertical lines mark standard deviation of measurement depths in each bin --- these lines tend to overlap each other. Each PDF is colored according to data coverage: one means that there is at least one hourly estimate for every hour in the season.
...
h1 = axx[2].plot(xx, xx/4, color='w', lw=2, ls='--')
h2 = axx[2].plot(xx, xx/4, color='k', lw=1, ls='--')
htxt = axx[2].text(xx[0], xx[0]/4, '    factor of 4')
...
** Supplementary Figures S1, S2, S3                                 :noexport:
#+BEGIN_SRC jupyter-python :session py
