...
[[file:images/paper1/figure5-shears.png]]
...
import xfilter
N2 = xfilter.lowpass(
    nrl5.N2.isel(depth=1).reset_coords(drop=True).interpolate_na("time"),
    coord="time",
    freq=1 / 30,
    cycles_per="D",
    num_discard=0,
...
f5.savefig('images/paper1/jim-paper1-nrl5.pdf', bbox_inches='tight')
...
ax[2].set_ylabel("Isothermal $S²$ [s$^{-2}$]")
ax[2].set_title("")
ax[2].set_xlabel("")
...
if "oaflux" not in locals():
    ...
    E = (
        (
            oaflux.evapr.sel(lon=slice(85, 89))
            .sel(lat=8, method="nearest")
            .drop("lat")
            .mean("lon")
        ).load()
        ,* 0.01
...
f.colorbar(
    ...
    label=r"hourly mean salt flux $ρ_0^{-1}$ $J_s^t$  [psu m/s]",
)
...
def _process_df(x, count_mask):
    ...
    return x.where(count_mask).reset_index().pivot("S", "time", "Js")
...
groupby = [pd.cut(df.S, [34, 34.5, 35, 35.5]), df.time.dt.month]
...
cmap = dcpy.plots.cmap_params(
    [0, 1, 2, 3, 4],
    levels=[0, 1, 2],
    cmap=mpl.colors.ListedColormap(
        [[0.3, 0.3, 0.3, 1], [0.6, 0.6, 0.6, 0.5], [0.3, 0.3, 0.3, 0]]
    ),
)
sns.heatmap(
    _process_df(count / N, count_mask),
    ,**cmap,
    ax=ax[-2],
    cbar=False,
    zorder=10,
...
[tt.set_rotation(0) for tt in ax[-2].get_yticklabels()]
ax[-2].set_ylim([3.5, 0])
ax[-2].set_ylabel("$S$ [psu]")

ax[-1].cla()
EP = (
    (32 * 1e6 * (E - P8.interp(time=E.time)))
    .sel(time="2014")
    .groupby("time.month")
    .mean("time")
)
...
ax[-1].legend(
    handles=[hep[1], herr],
    labels=["$S_0 (E-P)$", "$ρ_0^{-1}$ $J_s^t$ at $S$ = 34.75"],
    loc="lower center",
    frameon=True,
    framealpha=0.4,
    facecolor="lightgray",
...
ax[-1].set_ylabel("$ρ_0^{-1}$ $J_s^t$  [$10^{-6}$ psu m/s]")
...
dcpy.plots.label_subplots(
    ...
    x=0.03,
    y=0.1,
    labels=["", "", "$ρ_0^{-1}$ $J_s^t$ [$10^{-6}$ psu m/s]", ""],
    zorder=10,
)
...
f.savefig('images/paper1/figure10-8n.pdf', bbox_to_inches='tight')
#+END_SRC
