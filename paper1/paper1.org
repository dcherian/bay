...
1) The mixed and isothermal layer depths (MLD, ILD respectively) are defined using threshold criteria $Δρ > \SI{0.03}{\kg\per\m\cubed}$ and $ΔT > \SI{0.2}{\celsius}$ respectively. The mixed and barrier layer depths are only determinable at the RAMA moorings with CTDs at 1m, 10m, 20m, 40m, 60m and 100m. Measurements are within the mixed layer if the depth of the χpod $z_χ ≤ \text{MLD} + \SI{5}{m}$ and within the barrier layer if $\text{MLD} + \SI{5}{m} < z_χ ≤ \text{ILD} + \SI{5}{m}$[fn::NRL3 χpods appear to be within the mixed layer and barrier layers for a few weeks in February. There is only one CTD above these χpods, so we label NRL3 measurements as being within the mixed layer if $N^2 < \SI{5e-6}{\per\s\squared}$ and within the barrier layer if $T_z < \SI{2e-3}{\celsius\per\m}$ and $N^2 > \SI{5e-6}{\per\s\squared}$.].
...
3) All measurements are then binned by `ML', `BL' or by density with bin edges [1018, 1021, 1022, 1022.5, 1023, 1023.5, 1024.25, 1029] \SI{}{\kg\per\m\cubed}.
...
Near-molecular values of $K_T$ are observed during the NESW transition period --- mean $K_T ≤ \SI{1e-5}{\m\squared\per\s} (50κ_T)$ and median $K_T ≤ \SI{1e-6}{\m\squared\per\s} (5κ_T)$ for depths greater than \SI{50}{m} during March and April, assuming a typical value of molecular diffusivity $κ_T = \SI{2e-7}{\m\squared\per\s}$ (Figure ref:fig:vert).
...
Low values of $K_T$ are perhaps not surprising given the observations of cite:Jinadasa2016 and cite:Kunze2006 but these χpod observations are the first to show that extremely low mixing ($K_T ≤ 1\mhyph 10 κ_T$) persists for multiple weeks at multiple locations in the south-central Bay (Figure ref:fig:vert).
...
with mpl.style.context({'axes.titlesize': 9,
    ...
    for row in range(nrows):
        for col in range(ncols):
            axarr[row, col] = f.add_subplot(gsbot[row, col],
...
for season, aa in axes['big'].items():
    ...
    aa.set_xlabel('Longitude')
    dcpy.plots.liney(0, ax=aa)
[common(aa) for aa in axarr.ravel()]
...
cmapj = mpl.colors.LinearSegmentedColormap.from_list(
    name='j', colors=['w', '#5ABCE1', '#FFA500', '#ED2E00'])
# plt.figure(); eke.sel(monsoon='SW').plot(cmap=cmapj, robust=True)
...
for m in [nrl3, nrl4, nrl5]:
    ...
    lo = np.sum(fft.where(np.abs(fft.freq_time) < 0.1/86400) * fft.freq_time_spacing)
    ...
    total = np.var(shear.shear)

    hdl, _ = dcpy.ts.PlotSpectrum(shear.shear,
                                   ax=ax[m.name],
                                   twoside=False,
                                   preserve_area=True)
...
axx[0].set_ylim([0, 2.1e-5])
f.legend(handles=hdl, labels=['CW', 'CCW'], loc='upper right')

plt.tight_layout()

f.set_size_inches((39 * 1/6.0255, 5))

f.savefig('figure-xx-nrl-spectra.pdf', bbox_inches='tight')
#+END_SRC

#+CAPTION: Variance preserving spectra of shear at the deepest $\chi$pod's depth level. Lowpass, near-inertial and $M_2$ bands (shading) as well as percentage of total shear variance in each band (text) are shown.
[[file:figure-xx-nrl-spectra.pdf]]
