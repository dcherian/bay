...
for idx, m in enumerate([vel3["iso"], vel4["iso"], vel5["iso"]]):
    shear = m.shear.sel(T=19, method="nearest").sel(time="2014").interpolate_na("time")
    plot_spectrum(axx[idx, 1], shear)

for idx, m in enumerate([nrl3, nrl4, nrl5]):
    shear = (
        m.smooth_full_shear()
        .sel(time=slice("2013-12-01", "2014-12-01"))
        .shear.sel(depth=144, method="nearest")
    )
...
axx[0, 0].set_title("depth = 144m")

axx[0, 0].set_title(f"depth = {shear.depth.values}m", pad=20)
axx[0, 1].set_title("T = 19°C", pad=20)
...
f5.savefig('figure6-nrl5.pdf', bbox_inches='tight')
