...
with mpl.style.context({'axes.titlesize': 9,
    ...
    axes = dict()
    axes['big'] = dict()
    axes['big']['SWNE'] = f.add_subplot(gstop[0], projection=ccrs.PlateCarree())
...
for season, aa in axes['big'].items():
    ...
    hdl = mean_precips.precipitation.sel(monsoon=season).plot.contourf(
        ax=aa, levels=11, cmap=sns.color_palette("Blues", n_colors=11), add_labels=False,
        zorder=1, vmin=0, vmax=15, add_colorbar=False)
...
f.set_size_inches(dcpy.plots.pub_fig_width("jpo", "two column"), 8.2)
plt.tight_layout()
f.canvas.draw()
f.canvas.draw()
...
#+NAME: fig:map
#+ATTR_LATEX: :float multicolumn
#+CAPTION: 2014 \(\chi\)pod deployment at 8\(\degree\)N. (a) Locations of moorings with \(\chi\)pods. (b--i) \(\chi\)pod deployment depths marked using horizontal lines on seasonal mean temperature (b--e) and salinity (f--i) profiles from the Argo climatology, averaged between 85.5\(\degree\)E and 88.5\(\degree\)E. These moorings experienced significant blowdown. Horizontal lines and shading mark median and interquartile range of each \(\chi\)pod's depth. Salinity equal to 34.75 psu is marked by a black dot. Temperature and salinity axes (lower and upper x-axes) are scaled such that axis limits represent equal jumps in density.
...
#+NAME: fig:vert
#+ATTR_LATEX: :float multicolumn
#+CAPTION: \small The seasonal cycle of $K_T$ at 8°N. (top) Basin-wide seasonal averages of daily Tropflux wind stress $\tau$, near-inertial energy input $\Pi$, daily Tropflux net surface heat flux $J_q^0$ and TRMM precipitation $P$. Wind stress variability is represented by gray ellipses with widths and heights equal to twice the standard deviation of $\tau_x$ and $\tau_y$. (bottom) Vertical profile of hourly averaged $K_T$ formed by combining all estimates in density bins (Section [[sec:profile]]). PDFs as well as means and medians are shown. Bins are marked by $\rho-1000$. Orange horizontal lines mark the climatological depth of the $S=34.75$ isohaline at 8\(\degree\)N estimated using the Argo climatology. Vertical lines mark the standard deviation of measurement depths in each bin --- these lines tend to overlap each other. Each PDF is colored according to data coverage: one means that there is at least one hourly estimate for every hour in the season.
...
** Figure XX: NRL shears

#+NAME: fig:shears
#+ATTR_LATEX: :float multicolumn
#+CAPTION: Filtered shear components for the three moorings (left to right). From top to bottom, low-frequency shear, near-inertial shear, near-tidal shear and the residual.
...
if 'low' not in locals():
    ...
    for m in [nrl3, nrl4, nrl5]:
        ...
        _, l, _, n, _ = m.filter_interp_shear('filter')
