...
#+call: read-ra12()
#+BEGIN_SRC ipython :session :ipyfile images/ra12-new-summary.png
ax = ra12.Plotχpods(quiv=False, filt='hann', filter_len=5*86400.0,
                    TSkind='pcolor', met='tropflux')
ax['N2'].set_ylim([0, 8])
...
:RESULTS:
: (-100, 100)
[[file:images/ra12-new-summary.png]]
:END:
...
#+CALL: read-ra12()
#+BEGIN_SRC ipython :session :ipyfile images/ra12-daily.png
import dcpy.ts
...
dcpy.plots.offset_line_plot(frac_daily_samples, x='dayofyear', y='depth',
                            offset = 0, remove_mean=False)
#+END_SRC
#+RESULTS:
:RESULTS:
#+BEGIN_EXAMPLE
[<matplotlib.lines.Line2D at 0x7f2855644048>,
   <matplotlib.lines.Line2D at 0x7f2855644a20>,
   <matplotlib.lines.Line2D at 0x7f28556440b8>]
#+END_EXAMPLE
...
def summarize_sampling(var):
    def calc_hourly(da, igrp, groupstr):
        ...
        hourly = da.groupby('time.hour').apply(calc_fraction_sampled)
        hourly.coords[groupstr] = igrp
        hourly.name = 'Fraction sampled in an hour'
        return hourly
    ...
    for year, data in var.groupby('time.year'):
        ...
        fr = (xr.concat([calc_hourly(da, igrp, groupstr) for igrp,da in grouped],
                        dim=groupstr))
        fr.coords['year'] = year
        yrfrac.append(fr)

    frac = xr.concat(yrfrac, dim='year').drop([2013, 2016], dim='year')
    hdl = frac.plot(y='hour', row='year', col='depth', vmin=0, cmap=mpl.cm.Reds)
    plt.gcf().suptitle(ra12.name, y=1.01)
    return hdl

hdl = summarize_sampling(ra12.Jq)
#+END_SRC
#+CAPTION: Distribution of samples by hour on y axis. and month varying in x.
#+RESULTS:
[[file:images/ra12-jqt-sampling.png]]

#+BEGIN_SRC ipython :session :ipyfile images/ra12-jq0-N2-hourly.png
def hourly_means(da):
    return da.groupby('Jtime.hour').mean(dim='Jtime')

hdl = (ra12.met.Jq0.groupby('Jtime.weekofyear')
       .apply(hourly_means)
       .plot(x='weekofyear', y='hour'))
f = plt.gcf().set_size_inches((6.5, 3.5))
#+END_SRC
#+CAPTION: Net surface flux. Absolutely no change in time of solar forcing... Can see ISOs!
#+RESULTS:
[[file:images/ra12-jq0-N2-hourly.png]]
...
ax = ra15.Plotχpods(quiv=False, filt='hann', filter_len=5*86400.0,
                    TSkind='pcolor', met='tropflux')
ax['Jq'].set_ylim([-100, 100])
# ax['S'].set_ylim([30, 34])
ax['N2'].set_ylim([0, 7])
...
:RESULTS:
: (0, 7)
1[[file:images/ra15-summary.png]]
:END:
